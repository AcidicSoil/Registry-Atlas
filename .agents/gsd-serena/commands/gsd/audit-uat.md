---
name: gsd:audit-uat
description: "Use when operating the audit-uat bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge audit-uat --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:audit-uat"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/audit-uat.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-audit-uat`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge audit-uat --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `audit-uat`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   If the command needs arguments, inspect `gsd-serena-bridge audit-uat --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:audit-uat`
- Test fixture path: `tests/fixtures/runtime-capability/commands/audit-uat`
- Required command: `gsd-serena-bridge audit-uat --format markdown`
- Dry-run command: `gsd-serena-bridge audit-uat --format markdown`
- Apply command: `gsd-serena-bridge audit-uat --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/audit-uat.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/audit-uat.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/audit-uat.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/audit-uat.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-audit-uat`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/audit-uat.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/audit-uat.md`
...
- Bridge audit-uat renders the reference-shaped read-only UAT audit report from .planning/phases UAT and VERIFICATION artifacts.
- It emits All Clear, Testable Now, Needs Prerequisites, Stale, Recommended Actions, and Human UAT Test Plan sections without mutating artifacts or running tests.
- Runtime does not read HOME workflows or vendor reference files.
