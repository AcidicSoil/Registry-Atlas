---
name: gsd:settings
description: "Use when operating the settings bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge settings --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:settings"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/settings.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-settings`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge settings --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `settings`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   If the command needs arguments, inspect `gsd-serena-bridge settings --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:settings`
- Test fixture path: `tests/fixtures/runtime-capability/commands/settings`
- Required command: `gsd-serena-bridge settings --format markdown`
- Dry-run command: `gsd-serena-bridge settings --format markdown`
- Apply command: `gsd-serena-bridge settings --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/settings.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/settings.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/settings.md`.
...
- Contract ID: `gsd-command-settings`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/settings.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/settings.md`
...
- Bridge settings renders a reference-shaped read-only settings report over .planning/config.json.
- Interactive settings prompts, config writes, and profile switching remain blocked fail-closed because they mutate local configuration state.
- Bridge settings remains supported for read-only inspection and now includes a Serena settings operation plan with interactive answer checkpoints, config write-set, validation, and rollback.
