...
description: "Use when operating the validate-phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge validate-phase {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
  ...
  slashAliases:
    - "/gsd:validate-phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/validate-phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-validate-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge validate-phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `validate-phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge validate-phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge validate-phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:validate-phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/validate-phase`
- Required command: `gsd-serena-bridge validate-phase --format markdown`
- Dry-run command: `gsd-serena-bridge validate-phase --format markdown`
- Apply command: `gsd-serena-bridge validate-phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/validate-phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/validate-phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/validate-phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/validate-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-validate-phase`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/validate-phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/validate-phase.md`
...
- Bridge validate-phase reports validation evidence and now mirrors apply/repair mode as instruction guidance.
- Vendor validation repair intent is mirrored through missing-evidence lists, test command checklist, validation artifact repair plan, and state-update criteria without automatic writes, tests, agents, or workflow state mutation.
- Bridge validate-phase now includes a structured Serena phase-workflow operation plan with shell/test commands, phase artifact write-set, checkpoints, validation, rollback, and role workflow evidence while preserving existing safety gates.
