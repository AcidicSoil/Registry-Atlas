---
name: gsd:autonomous
description: "Use when operating the autonomous bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge autonomous --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:autonomous"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/autonomous.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-autonomous`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge autonomous --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `autonomous`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge autonomous --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge autonomous --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:autonomous`
- Test fixture path: `tests/fixtures/runtime-capability/commands/autonomous`
- Required command: `gsd-serena-bridge autonomous --format markdown`
- Dry-run command: `gsd-serena-bridge autonomous --format markdown`
- Apply command: `gsd-serena-bridge autonomous --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/autonomous.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/autonomous.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/autonomous.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/autonomous.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Native query translated: `Project context, phase list, and state are resolved inside the workflow using init commands ('gsd-tools query init.milestone-op', 'gsd-tools query roadmap.analyze'). No upfront context loading needed.` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
</context>
...
- Contract ID: `gsd-command-autonomous`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/autonomous.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/autonomous.md`
...
- Bridge autonomous reports phase-window readiness and now mirrors interactive dispatch mode as instruction guidance.
- Vendor autonomous intent is mirrored through phase dispatch order, required reads/writes, validation commands, stop conditions, and checkpoint guidance without automatic autonomous execution.
- Bridge autonomous now includes a structured Serena memory/session/manager/autonomy operation plan with bounded local state, write sets, command budgets, external integration boundaries, checkpoints, rollback, and session/memory/triage/autonomy role workflow evidence while preserving existing safety gates.
