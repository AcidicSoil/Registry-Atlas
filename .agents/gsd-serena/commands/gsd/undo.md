---
name: gsd:undo
description: "Use when operating the undo bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge undo {{plan}} --format markdown"
  variables:
    - name: "plan"
      required: false
      placeholder: "{{plan}}"
      description: "Plan id used to select one undo candidate."
  slashAliases:
    - "/gsd:undo"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/undo.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-undo`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge undo --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `undo`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge undo --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge undo --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:undo`
- Test fixture path: `tests/fixtures/runtime-capability/commands/undo`
- Required command: `gsd-serena-bridge undo --format markdown`
- Dry-run command: `gsd-serena-bridge undo --format markdown`
- Apply command: `gsd-serena-bridge undo --plan <plan> --apply --confirm-local-git --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/commands/undo.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/undo.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/undo.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/undo.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- `vendor/reference/gsd-core/commands/gsd/undo.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/undo.md`
...
- Bridge undo now performs exact-safe local git revert behavior behind --confirm-local-git.
- Dry-run mode remains read-only and previews recent, phase, or plan-scoped candidate commits.
- Confirmed apply mode uses git revert --no-commit for the selected --last single commit or all matching --phase/--plan commits, leaving final commit/abort decisions to the operator.
- Reset, checkout, history rewrite, remote push, and interactive confirmation prompts remain out of scope.
