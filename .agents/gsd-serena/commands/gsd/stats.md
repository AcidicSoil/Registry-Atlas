---
name: gsd:stats
description: "Use when operating the stats bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge stats --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:stats"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/stats.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-stats`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge stats --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `stats`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge stats --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge stats --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:stats`
- Test fixture path: `tests/fixtures/runtime-capability/commands/stats`
- Required command: `gsd-serena-bridge stats --format markdown`
- Dry-run command: `gsd-serena-bridge stats --format markdown`
- Apply command: `gsd-serena-bridge stats --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/stats.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/stats.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/stats.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/stats.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/stats.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/stats.md`
...
- Bridge stats renders reference-shaped project statistics from local .planning files and read-only git metadata.
- It emits progress, plans, phase table, requirements, git, and timeline sections plus missing-planning guidance matching the reference workflow intent.
- Runtime does not invoke external gsd-tools, npx, HOME workflows, or vendor reference files and does not mutate project state.
