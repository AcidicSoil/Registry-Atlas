---
name: gsd:audit-fix
description: "Use when operating the audit-fix bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge audit-fix --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:audit-fix"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/audit-fix.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-audit-fix`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge audit-fix --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `audit-fix`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge audit-fix --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge audit-fix --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:audit-fix`
- Test fixture path: `tests/fixtures/runtime-capability/commands/audit-fix`
- Required command: `gsd-serena-bridge audit-fix --format markdown`
- Dry-run command: `gsd-serena-bridge audit-fix --format markdown`
- Apply command: `gsd-serena-bridge audit-fix --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/AUDIT_FIX.md`
- External actions:
- none
- Git actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/audit-fix.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/audit-fix.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/audit-fix.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/audit-fix.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-audit-fix`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/audit-fix.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/audit-fix.md`
...
- Bridge audit-fix reports audit finding classifications and now mirrors apply/commit modes as instruction guidance.
- Vendor fix intent is mirrored through finding evidence, proposed patch, validation command, rollback note, and commit-readiness checklist without automatic source edits, tests, agents, or commits.
- Bridge audit-fix now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
