---
name: gsd:thread
description: "Use when operating the thread bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge thread {{subcommand}} --format markdown"
  variables:
    - name: "subcommand"
      required: false
      placeholder: "{{subcommand}}"
      description: "Thread subcommand or description."
  slashAliases:
    - "/gsd:thread"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/thread.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-thread`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge thread --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `thread`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge thread --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge thread --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:thread`
- Test fixture path: `tests/fixtures/runtime-capability/commands/thread`
- Required command: `gsd-serena-bridge thread --format markdown`
- Dry-run command: `gsd-serena-bridge thread --format markdown`
- Apply command: `gsd-serena-bridge thread --write --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/thread.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/thread.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/thread.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/thread.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-thread`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/thread.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/thread.md`
...
- Bridge thread supports reference list/default, list --open, list --resolved, status, close, resume, and create-by-description modes over local .planning/threads files.
- Runtime writes are limited to .planning/threads/{slug}.md with strict slug sanitization and plain-text content rendering.
- Runtime does not depend on external gsd-tools, HOME workflows, shell commit, or agent dispatch.
