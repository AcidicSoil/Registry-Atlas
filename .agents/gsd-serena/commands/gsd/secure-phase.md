...
description: "Use when operating the secure-phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge secure-phase {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
  ...
  slashAliases:
    - "/gsd:secure-phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/secure-phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-secure-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge secure-phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `secure-phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge secure-phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge secure-phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:secure-phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/secure-phase`
- Required command: `gsd-serena-bridge secure-phase --format markdown`
- Dry-run command: `gsd-serena-bridge secure-phase --format markdown`
- Apply command: `gsd-serena-bridge secure-phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/SECURE_PHASE.md`
- External actions:
- none
- Git actions:
- none
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/secure-phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/secure-phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/secure-phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/secure-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-secure-phase`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/secure-phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/secure-phase.md`
...
- Bridge secure-phase reports security readiness and now mirrors write/edit/agent/run modes as instruction guidance.
- Vendor security intent is mirrored through SECURITY.md write-set, threat review checklist, validation criteria, and rollback notes without automatic artifact mutation; audits, scanners, and role workflows remain explicit Serena operations.
- Bridge secure-phase now includes a structured Serena phase-workflow operation plan with shell/test commands, phase artifact write-set, checkpoints, validation, rollback, and role workflow evidence while preserving existing safety gates.
