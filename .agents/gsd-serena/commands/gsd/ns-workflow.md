...
description: "Use when operating the ns-workflow bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ns-workflow --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ns-workflow"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ns-workflow.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ns-workflow`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ns-workflow --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ns-workflow`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ns-workflow --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ns-workflow --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ns-workflow`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ns-workflow`
- Required command: `gsd-serena-bridge ns-workflow --format markdown`
- Dry-run command: `gsd-serena-bridge ns-workflow --format markdown`
- Apply command: `gsd-serena-bridge ns-workflow --write --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ns-workflow.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ns-workflow.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ns-workflow.md`.
...
- Contract ID: `gsd-command-ns-workflow`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ns-workflow.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ns-workflow.md`
...
- Bridge ns-workflow is adapted as a read-only phase-pipeline namespace routing readiness report with vendor-listed routes for discuss-phase, spec-phase, plan-phase, execute-phase, verify-work, phase, progress, ultraplan-phase, plan-review-convergence, add-tests, ai-integration-phase, autonomous, fast, mvp-phase, and quick.
- It reports likely route matches, selected route when unambiguous, required route command coverage, and local roadmap/state/phase/PLAN/UAT evidence.
- It does not invoke Skill, run workflow commands, create or mutate phase/PLAN/UAT/roadmap/state/cloud-planning/convergence/test/AI/MVP/quick-task artifacts, ask routing questions, or perform interactive namespace management; dispatch execution is staged for namespace/Skill operation parity, so status remains adapted-safe.
- Bridge ns-workflow now includes a structured Serena namespace/skill dispatch operation plan with route selection, command bundle evidence, skill-equivalent instruction modules, role workflows, checkpoints, validation, rollback, and dispatch ledger write-set evidence while preserving existing safety gates.
