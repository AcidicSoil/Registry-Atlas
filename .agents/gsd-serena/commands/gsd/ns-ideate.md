---
name: gsd:ns-ideate
description: "Use when operating the ns-ideate bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ns-ideate --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ns-ideate"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ns-ideate.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ns-ideate`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ns-ideate --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ns-ideate`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ns-ideate --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ns-ideate --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ns-ideate`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ns-ideate`
- Required command: `gsd-serena-bridge ns-ideate --format markdown`
- Dry-run command: `gsd-serena-bridge ns-ideate --format markdown`
- Apply command: `gsd-serena-bridge ns-ideate --write --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ns-ideate.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ns-ideate.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ns-ideate.md`.
...
- Contract ID: `gsd-command-ns-ideate`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ns-ideate.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ns-ideate.md`
...
- Bridge ns-ideate is adapted as a read-only exploration/capture namespace routing readiness report with vendor-listed routes for explore, sketch, spike, spec-phase, and capture.
- It reports likely route matches, selected route when unambiguous, required route command coverage, and local planning evidence.
- It does not invoke Skill, run mapped commands, create notes/backlog/seeds/spikes/specs/phase artifacts, ask routing questions, or perform interactive namespace management; dispatch execution is staged for namespace/Skill operation parity, so status remains adapted-safe.
- Bridge ns-ideate now includes a structured Serena namespace/skill dispatch operation plan with route selection, command bundle evidence, skill-equivalent instruction modules, role workflows, checkpoints, validation, rollback, and dispatch ledger write-set evidence while preserving existing safety gates.
