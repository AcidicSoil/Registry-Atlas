---
name: gsd:update
description: "Use when operating the update bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge update --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:update"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/update.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-update`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge update --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `update`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge update --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge update --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:update`
- Test fixture path: `tests/fixtures/runtime-capability/commands/update`
- Required command: `gsd-serena-bridge update --format markdown`
- Dry-run command: `gsd-serena-bridge update --format markdown`
- Apply command: `gsd-serena-bridge update --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/update.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/update.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/update.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/update.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- **--next** (alias **--rc**): Target the `@next` RC dist-tag instead of `@latest` so you can install or refresh a release candidate (e.g. `1.4.0-rc.1`) through the normal update flow — scope/runtime detection, changelog preview, custom-file backup, and cac...
...
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/sync-skills.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/reapply-patches.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context_extended>

## Contract Reference

- Contract ID: `gsd-command-update`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/update.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/update.md`
...
- Bridge update now supports exact-safe managed-surface sync through the repair lifecycle.
- Standard/latest/next modes remain read-only readiness reports and do not query npm or install packages.
- --sync refreshes bridge-owned installed surfaces, updates install-state hashes, and preserves divergent local edits as pending updates or local patches.
- --reapply remains guidance-only; local patch hunk reapplication is still explicit operator work.
