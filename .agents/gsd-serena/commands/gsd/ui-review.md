---
name: gsd:ui-review
description: "Use when operating the ui-review bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ui-review --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ui-review"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ui-review.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ui-review`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ui-review --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ui-review`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ui-review --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ui-review --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ui-review`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ui-review`
- Required command: `gsd-serena-bridge ui-review --format markdown`
- Dry-run command: `gsd-serena-bridge ui-review --format markdown`
- Apply command: `gsd-serena-bridge ui-review --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/UI_REVIEW.md`
- External actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ui-review.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ui-review.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ui-review.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/ui-review.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-ui-review`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ui-review.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ui-review.md`
...
- Bridge ui-review reports local UI readiness and now mirrors write/run/agent/audit modes as visual-audit instruction guidance.
- Vendor UI review intent is mirrored through pillar, accessibility, responsiveness, interaction, brand, and consistency guidance using Serena role-workflow guidance instead of vendor-native subagent dispatch, browser tooling, grades, or UI-REVIEW writes.
- Bridge ui-review now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
