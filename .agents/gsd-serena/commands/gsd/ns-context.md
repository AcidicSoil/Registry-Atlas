---
name: gsd:ns-context
description: "Use when operating the ns-context bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ns-context --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ns-context"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ns-context.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ns-context`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ns-context --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ns-context`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ns-context --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ns-context --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ns-context`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ns-context`
- Required command: `gsd-serena-bridge ns-context --format markdown`
- Dry-run command: `gsd-serena-bridge ns-context --format markdown`
- Apply command: `gsd-serena-bridge ns-context --write --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ns-context.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ns-context.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ns-context.md`.
...
- Contract ID: `gsd-command-ns-context`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ns-context.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ns-context.md`
...
- Bridge ns-context is adapted as a read-only context namespace routing readiness report with vendor-listed routes for map-codebase, map-codebase --fast, map-codebase --query, graphify, docs-update, extract-learnings, mempalace-recall, and mempalace-capture.
- It reports likely route matches, selected route when unambiguous, required route command coverage, and local context evidence directories.
- It does not invoke Skill, run mapped commands, create intelligence artifacts, ask routing questions, or perform interactive namespace management; dispatch execution is staged for namespace/Skill operation parity, so status remains adapted-safe.
- Bridge ns-context now includes a structured Serena namespace/skill dispatch operation plan with route selection, command bundle evidence, skill-equivalent instruction modules, role workflows, checkpoints, validation, rollback, and dispatch ledger write-set evidence while preserving existing safety gates.
