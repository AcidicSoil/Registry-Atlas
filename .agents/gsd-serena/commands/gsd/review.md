---
name: gsd:review
description: "Use when operating the review bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge review {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
      description: "Phase directory name supplied by --phase."
  slashAliases:
    - "/gsd:review"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/review.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-review`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge review --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `review`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge review --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge review --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:review`
- Test fixture path: `tests/fixtures/runtime-capability/commands/review`
- Required command: `gsd-serena-bridge review --format markdown`
- Dry-run command: `gsd-serena-bridge review --format markdown`
- Apply command: `gsd-serena-bridge review --phase <phase> --agent --confirm-role-runner --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/commands/review.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/review.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/review.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/review.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-review`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/review.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/review.md`
...
- Bridge review reports phase/reviewer readiness and now mirrors write/run/agent modes as external-review instruction guidance.
- Vendor multi-reviewer behavior is mirrored through reviewer-specific prompt guidance and proposed REVIEWS.md collection steps without external CLI calls or writes.
- Bridge review now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
