---
name: gsd:pr-branch
description: "Use when operating the pr-branch bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge pr-branch {{target}} --format markdown"
  variables:
    - name: "target"
      required: false
      placeholder: "{{target}}"
  ...
  slashAliases:
    - "/gsd:pr-branch"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/pr-branch.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-pr-branch`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge pr-branch --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `pr-branch`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge pr-branch --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge pr-branch --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:pr-branch`
- Test fixture path: `tests/fixtures/runtime-capability/commands/pr-branch`
- Required command: `gsd-serena-bridge pr-branch --format markdown`
- Dry-run command: `gsd-serena-bridge pr-branch --format markdown`
- Apply command: `gsd-serena-bridge pr-branch main --create --branch <name> --confirm-local-git --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/commands/pr-branch.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/pr-branch.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/pr-branch.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/pr-branch.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-pr-branch`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/pr-branch.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/pr-branch.md`
...
- Bridge pr-branch now performs exact-safe local clean branch creation behind --confirm-local-git.
- Dry-run mode remains read-only and classifies code vs planning artifact changes relative to the target branch.
- Confirmed mutation mode creates a local branch from the target branch, reapplies only non-planning code changes, commits them, and leaves remote push/history rewrite out of scope.
