...
description: "Use when operating the extract-learnings bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge extract-learnings --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:extract-learnings"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/extract-learnings.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-extract-learnings`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge extract-learnings --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `extract-learnings`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge extract-learnings --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge extract-learnings --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:extract-learnings`
- Test fixture path: `tests/fixtures/runtime-capability/commands/extract-learnings`
- Required command: `gsd-serena-bridge extract-learnings --format markdown`
- Dry-run command: `gsd-serena-bridge extract-learnings --format markdown`
- Apply command: `gsd-serena-bridge extract-learnings --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/extract-learnings.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/extract-learnings.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/extract-learnings.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/extract-learnings.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-extract-learnings`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/extract-learnings.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/extract-learnings.md`
...
- Bridge extract-learnings reports phase learning candidates and now mirrors write/agent modes as instruction guidance.
- Vendor learning extraction intent is mirrored through decisions, lessons, patterns, surprises, risks, gaps, and follow-up TODO guidance without LEARNINGS writes or extraction agents.
- Bridge extract-learnings now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
