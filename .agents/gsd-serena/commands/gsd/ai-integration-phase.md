...
description: "Use when operating the ai-integration-phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ai-integration-phase --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ai-integration-phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ai-integration-phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ai-integration-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ai-integration-phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ai-integration-phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ai-integration-phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ai-integration-phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ai-integration-phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ai-integration-phase`
- Required command: `gsd-serena-bridge ai-integration-phase --format markdown`
- Dry-run command: `gsd-serena-bridge ai-integration-phase --format markdown`
- Apply command: `gsd-serena-bridge ai-integration-phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/AI_INTEGRATION_PHASE.md`
- External actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ai-integration-phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ai-integration-phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ai-integration-phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/ai-integration-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-ai-integration-phase`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ai-integration-phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ai-integration-phase.md`
...
- Bridge ai-integration-phase reports AI design readiness and now mirrors write/research/web/agent modes as instruction guidance.
- Vendor AI integration intent is mirrored through AI-SPEC write-set, research questions, evaluation strategy, dataset/retrieval notes, and validation criteria without automatic artifact writes; web tools and role workflows remain explicit Serena operations.
- Bridge ai-integration-phase now includes a structured Serena phase-workflow operation plan with shell/test commands, phase artifact write-set, checkpoints, validation, rollback, and role workflow evidence while preserving existing safety gates.
