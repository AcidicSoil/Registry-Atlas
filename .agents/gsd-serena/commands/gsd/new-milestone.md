...
description: "Use when operating the new-milestone bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge new-milestone {{title}} --format markdown"
  variables:
    - name: "title"
      required: true
      placeholder: "{{title}}"
  ...
  slashAliases:
    - "/gsd:new-milestone"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/new-milestone.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-new-milestone`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge new-milestone --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `new-milestone`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   If the command needs arguments, inspect `gsd-serena-bridge new-milestone --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:new-milestone`
- Test fixture path: `tests/fixtures/runtime-capability/commands/new-milestone`
- Required command: `gsd-serena-bridge new-milestone --format markdown`
- Dry-run command: `gsd-serena-bridge new-milestone --format markdown`
- Apply command: `gsd-serena-bridge new-milestone --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/new-milestone.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/new-milestone.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/new-milestone.md`.
...
**After:** ``gsd-serena-bridge plan-phase --format markdown` [N]` to start execution.
</objective>

<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/new-milestone.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-new-milestone`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/new-milestone.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/new-milestone.md`
...
- Supported for bridge-owned milestone creation: validates active workspace/workstream scope, rejects duplicate active titles, and writes only .planning/.bridge/milestones.json. Native questioning/research/ROADMAP/REQUIREMENTS/STATE writes remain out of scope.
