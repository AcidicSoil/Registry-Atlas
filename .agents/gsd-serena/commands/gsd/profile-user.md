...
description: "Use when operating the profile-user bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge profile-user --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:profile-user"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/profile-user.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-profile-user`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge profile-user --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `profile-user`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge profile-user --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge profile-user --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:profile-user`
- Test fixture path: `tests/fixtures/runtime-capability/commands/profile-user`
- Required command: `gsd-serena-bridge profile-user --format markdown`
- Dry-run command: `gsd-serena-bridge profile-user --format markdown`
- Apply command: `gsd-serena-bridge profile-user --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/profile-user.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/profile-user.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/profile-user.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/profile-user.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-profile-user`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/profile-user.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/profile-user.md`
...
- Bridge profile-user reports local personalization evidence and now mirrors write/agent/apply modes as consent-first instruction guidance.
- Vendor profiling intent is mirrored through explicit preference collection, draft artifacts, human review, and reversible apply criteria without inferring private session traits automatically.
- Bridge profile-user now includes a structured Serena exploration/research/prototype operation plan with bounded tool use, artifact write sets, shell verification, checkpoints, rollback, and researcher/prototype/reviewer role workflow evidence while preserving existing safety gates.
