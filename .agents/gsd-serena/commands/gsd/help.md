---
name: gsd:help
description: "Use when operating the help bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge help {{topic}} --format markdown"
  variables:
    - name: "topic"
      required: false
      placeholder: "{{topic}}"
  ...
  slashAliases:
    - "/gsd:help"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/help.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-help`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge help --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `help`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge help --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge help --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:help`
- Test fixture path: `tests/fixtures/runtime-capability/commands/help`
- Required command: `gsd-serena-bridge help --format markdown`
- Dry-run command: `gsd-serena-bridge help --format markdown`
- Apply command: `gsd-serena-bridge help --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/help.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/help.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/help.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/help.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/help.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/help.md`
...
- Bridge help renders source-backed reference help modes from packageable assets for default, brief, full, topic, compact-topic, and unknown-topic output.
- Markdown output emits only selected reference help content; JSON wraps the same content for machine use.
- Runtime does not read $HOME/.claude or gitignored vendor reference files.
