---
name: gsd:health
description: "Use when operating the health bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge health --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:health"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/health.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-health`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge health --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `health`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge health --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge health --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:health`
- Test fixture path: `tests/fixtures/runtime-capability/commands/health`
- Required command: `gsd-serena-bridge health --format markdown`
- Dry-run command: `gsd-serena-bridge health --format markdown`
- Apply command: `gsd-serena-bridge health --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/health.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/health.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/health.md`.

<objective>
Validate `.planning/` directory integrity and report actionable issues. Checks for missing files, invalid configurations, inconsistent state, and orphaned plans.

- Native query translated: `'--context' runs an orthogonal check: the running session's context utilization. The workflow asks for the model's tokensUsed + contextWindow, calls 'gsd-tools query validate.context', and renders one of three states:` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/health.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-health`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/health.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/health.md`
...
- Bridge health renders a reference-shaped read-only planning integrity report over local .planning evidence.
- The supported slice covers default health output with HEALTHY, DEGRADED, and BROKEN states plus errors, warnings, info, and repairable counts.
- Repair, backfill, and context modes remain blocked fail-closed because they write files, backfill milestone state, or require interactive/runtime telemetry.
