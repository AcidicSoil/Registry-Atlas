---
name: gsd:capture
description: "Use when operating the capture bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge capture {{text}} --format markdown"
  variables:
    - name: "text"
      required: false
      placeholder: "{{text}}"
      description: "Optional note or capture text."
  slashAliases:
    - "/gsd:capture"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/capture.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-capture`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge capture --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `capture`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge capture --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge capture --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:capture`
- Test fixture path: `tests/fixtures/runtime-capability/commands/capture`
- Required command: `gsd-serena-bridge capture --format markdown`
- Dry-run command: `gsd-serena-bridge capture --format markdown`
- Apply command: `gsd-serena-bridge capture --write --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/capture.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/capture.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/capture.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/add-todo.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/note.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/add-backlog.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/plant-seed.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/check-todos.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/list-seeds.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-capture`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/capture.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/capture.md`
...
- Bridge capture still lists local todos read-only and now mirrors note/backlog/seed/default text capture modes as instruction guidance.
- Vendor capture intent is mirrored through destination selection, tags, deduplication, promotion route, and rollback guidance without automatic planning writes; default text capture is documented in notes because it is positional input rather than a scoped flag.
- Bridge capture now includes a structured Serena memory/session/manager/autonomy operation plan with bounded local state, write sets, command budgets, external integration boundaries, checkpoints, rollback, and session/memory/triage/autonomy role workflow evidence while preserving existing safety gates.
