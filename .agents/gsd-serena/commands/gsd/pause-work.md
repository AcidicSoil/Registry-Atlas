---
name: gsd:pause-work
description: "Use when operating the pause-work bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge pause-work --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:pause-work"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/pause-work.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-pause-work`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge pause-work --format markdown`.
- For durable handoff, prefer: `gsd-serena-bridge pause-work --save-handoff --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `pause-work`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge pause-work --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge pause-work --help` or use the resolver packet instead of guessing.
...
- `pnpm vitest run tests/gsd-serena/commands/pause-resume.test.ts`
...
- `.planning/.bridge/handoff-ledger/current.json`

## Runtime Capability

- Runtime status: `destructive-with-confirmation`
- Contract row: `command:pause-work`
- Test fixture path: `tests/fixtures/runtime-capability/commands/pause-work`
- Required command: `gsd-serena-bridge pause-work --format markdown`
- Dry-run command: `gsd-serena-bridge pause-work --format markdown`
- Apply command: `gsd-serena-bridge pause-work --apply --confirm --format markdown`
- Read actions:
- `.planning/HANDOFF.json`
- `.planning/.bridge/workstreams.json`
- `.planning/STATE.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/pause-work.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/pause-work.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/pause-work.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/pause-work.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-pause-work`
- Family: `command`
- Status: `adapted-safe`
- Runtime authority: `.agents/gsd-serena/parity-skills/contracts.json`
- Transition rule: adapted-safe — Bridge command supports bridge-owned workstream pause and optional handoff artifacts; native WIP git commit behavior remains intentionally out of scope.
...
- `vendor/reference/gsd-core/commands/gsd/pause-work.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/pause-work.md`
...
- Commands:
- `pnpm vitest run tests/gsd-serena/commands/pause-resume.test.ts`
- Notes: Covers pause-work/resume-work bridge-owned workstream metadata, terminal workstream rejection, idempotency, and optional --save-handoff artifacts.

### Notes

- Supported for bridge-owned workstream pause: validates active bridge metadata, rejects terminal workstreams, is idempotent when already paused, and by default writes only .planning/.bridge/workstreams.json. Explicit --save-handoff additionally writes .planning/HANDOFF.json, .planning/.continue-here.md, and .planning/.bridge/handoff-ledger/current.json. Native WIP commit behavior remains out of scope.
