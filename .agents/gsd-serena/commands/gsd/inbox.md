---
name: gsd:inbox
description: "Use when operating the inbox bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge inbox --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:inbox"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/inbox.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-inbox`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge inbox --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `inbox`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge inbox --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge inbox --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:inbox`
- Test fixture path: `tests/fixtures/runtime-capability/commands/inbox`
- Required command: `gsd-serena-bridge inbox --format markdown`
- Dry-run command: `gsd-serena-bridge inbox --format markdown`
- Apply command: `gsd-serena-bridge inbox --repo <owner/repo> --item <issue-or-pr> --label --close-incomplete --apply --fake-github --confirm-external --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/commands/inbox.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/inbox.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/inbox.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/inbox.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/inbox.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/inbox.md`
...
- Bridge inbox reports repository/template readiness and now mirrors label/close-incomplete modes as instruction guidance.
- Vendor triage intent is mirrored through issue/PR classification, completeness checks, labels, comments, close rationale, and evidence without automatic gh/GitHub mutations.
- Bridge inbox now includes a structured Serena memory/session/manager/autonomy operation plan with bounded local state, write sets, command budgets, external integration boundaries, checkpoints, rollback, and session/memory/triage/autonomy role workflow evidence while preserving existing safety gates.
