---
name: gsd:add-tests
description: "Use when operating the add-tests bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge add-tests {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
  ...
  slashAliases:
    - "/gsd:add-tests"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/add-tests.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-add-tests`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge add-tests --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `add-tests`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge add-tests --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge add-tests --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:add-tests`
- Test fixture path: `tests/fixtures/runtime-capability/commands/add-tests`
- Required command: `gsd-serena-bridge add-tests --format markdown`
- Dry-run command: `gsd-serena-bridge add-tests --format markdown`
- Apply command: `gsd-serena-bridge add-tests --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/ADD_TESTS.md`
- External actions:
- none
- Git actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/add-tests.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/add-tests.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/add-tests.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/add-tests.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>

<context>
Phase: $ARGUMENTS

- Target-project runtime reference retained: `@.planning/STATE.md`. Resolve it in the target project at operation time; do not bake source-repository planning content into generated assets.
- Target-project runtime reference retained: `@.planning/ROADMAP.md`. Resolve it in the target project at operation time; do not bake source-repository planning content into generated assets.
</context>
...
- Contract ID: `gsd-command-add-tests`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/add-tests.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/add-tests.md`
...
- Bridge add-tests reports phase test-planning evidence and now mirrors generation/commit modes as instruction guidance.
- Vendor test generation intent is mirrored through candidate files, verification markers, draft test cases, RED/GREEN checklist, and commit readiness without writing or executing tests.
- Bridge add-tests now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
