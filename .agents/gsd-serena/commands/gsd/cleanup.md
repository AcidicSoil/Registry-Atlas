---
name: gsd:cleanup
description: "Use when operating the cleanup bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge cleanup --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:cleanup"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/cleanup.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-cleanup`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge cleanup --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `cleanup`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   If the command needs arguments, inspect `gsd-serena-bridge cleanup --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:cleanup`
- Test fixture path: `tests/fixtures/runtime-capability/commands/cleanup`
- Required command: `gsd-serena-bridge cleanup --format markdown`
- Dry-run command: `gsd-serena-bridge cleanup --format markdown`
- Apply command: `gsd-serena-bridge cleanup --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/cleanup.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/cleanup.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/cleanup.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/cleanup.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-cleanup`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/cleanup.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/cleanup.md`
...
- Bridge cleanup renders a reference-shaped cleanup report over planning phase and milestone directories.
- Archive/apply now derives phase membership from .planning/milestones/v*-ROADMAP.md, skips existing v*-phases archives and likely backlog phases, writes .planning/CLEANUP.md, and moves matched phase directories only when explicitly confirmed.
- It does not update ROADMAP.md or STATE.md, create commits/tags, or prune branches automatically; missing milestone evidence produces a blocked archive plan.
