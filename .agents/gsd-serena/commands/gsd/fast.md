---
name: gsd:fast
description: "Use when operating the fast bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge fast --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:fast"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/fast.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-fast`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge fast --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `fast`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge fast --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge fast --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:fast`
- Test fixture path: `tests/fixtures/runtime-capability/commands/fast`
- Required command: `gsd-serena-bridge fast --format markdown`
- Dry-run command: `gsd-serena-bridge fast --format markdown`
- Apply command: `gsd-serena-bridge fast --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/FAST.md`
- External actions:
- none
- Git actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/fast.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/fast.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/fast.md`.
...
This is NOT a replacement for `gsd-serena-bridge quick --format markdown` — use `gsd-serena-bridge quick --format markdown` for anything that
needs research, multi-step planning, or verification. `gsd-serena-bridge fast --format markdown` is for tasks
you could describe in one sentence and execute in under 2 minutes.
</objective>

<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/fast.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/fast.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/fast.md`
...
- Bridge fast now provides read-only task classification plus instruction-guidance parity for execution/edit/bash modes.
- Vendor inline execution intent is mirrored as manual steps for small reversible edits, validation, and commit readiness without mutating files or running commands.
- Bridge fast now includes a structured Serena exploration/research/prototype operation plan with bounded tool use, artifact write sets, shell verification, checkpoints, rollback, and researcher/prototype/reviewer role workflow evidence while preserving existing safety gates.
