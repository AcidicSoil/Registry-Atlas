---
name: gsd:mvp-phase
description: "Use when operating the mvp-phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge mvp-phase {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
      description: "Phase number or phase directory name."
  slashAliases:
    - "/gsd:mvp-phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/mvp-phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-mvp-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge mvp-phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `mvp-phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge mvp-phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge mvp-phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:mvp-phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/mvp-phase`
- Required command: `gsd-serena-bridge mvp-phase --format markdown`
- Dry-run command: `gsd-serena-bridge mvp-phase --format markdown`
- Apply command: `gsd-serena-bridge mvp-phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/mvp-phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/mvp-phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/mvp-phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/mvp-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-mvp-phase`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/mvp-phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/mvp-phase.md`
...
- Bridge mvp-phase reports MVP conversion readiness and now mirrors write/delegate/agent modes as instruction guidance.
- Vendor MVP conversion intent is mirrored through user-story prompts, SPIDR split decisions, ROADMAP write-set, and delegation notes without automatic roadmap edits or phase creation; role workflows and plan-phase dispatch remain explicit Serena operations.
- Bridge mvp-phase now includes a structured Serena phase-workflow operation plan with shell/test commands, phase artifact write-set, checkpoints, validation, rollback, and role workflow evidence while preserving existing safety gates.
