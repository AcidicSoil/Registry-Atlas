---
name: gsd:debug
description: "Use when operating the debug bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge debug {{slug}} --format markdown"
  variables:
    - name: "slug"
      required: false
      placeholder: "{{slug}}"
      description: "Optional debug session slug or description."
  slashAliases:
    - "/gsd:debug"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/debug.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-debug`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge debug --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `debug`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge debug --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge debug --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:debug`
- Test fixture path: `tests/fixtures/runtime-capability/commands/debug`
- Required command: `gsd-serena-bridge debug --format markdown`
- Dry-run command: `gsd-serena-bridge debug --format markdown`
- Apply command: `gsd-serena-bridge debug --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/debug.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/debug.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/debug.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/debug.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/debug.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/debug.md`
...
- Bridge debug is adapted as read-only debug session inspection for list/status plus instruction-guidance parity for debug, continue, and --diagnose flows.
- Debug guidance mirrors gsd-debug-session-manager and gsd-debugger intent through scientific-method steps, continuation checkpoints, and root-cause-report guidance without automatically writing .planning/debug files, using Serena role-workflow guidance instead of vendor-native subagent dispatch.
- Native debug agents, shell/test execution, interactive questioning, and debug artifact writes remain unfinished parity work with future write-set/checkpoint/agent contracts.
- Bridge debug now includes a structured Serena memory/session/manager/autonomy operation plan with bounded local state, write sets, command budgets, external integration boundaries, checkpoints, rollback, and session/memory/triage/autonomy role workflow evidence while preserving existing safety gates.
