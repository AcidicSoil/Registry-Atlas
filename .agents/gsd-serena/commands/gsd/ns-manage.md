---
name: gsd:ns-manage
description: "Use when operating the ns-manage bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ns-manage --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:ns-manage"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ns-manage.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ns-manage`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ns-manage --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ns-manage`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ns-manage --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ns-manage --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ns-manage`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ns-manage`
- Required command: `gsd-serena-bridge ns-manage --format markdown`
- Dry-run command: `gsd-serena-bridge ns-manage --format markdown`
- Apply command: `gsd-serena-bridge ns-manage --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ns-manage.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ns-manage.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ns-manage.md`.
...
- Contract ID: `gsd-command-ns-manage`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ns-manage.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ns-manage.md`
...
- Bridge ns-manage is adapted as a read-only management namespace routing readiness report with vendor-listed routes for config, workspace, workstreams, thread, pause-work, resume-work, update, ship, inbox, pr-branch, undo, cleanup, health, manager, settings, stats, surface, and help.
- It reports likely route matches, selected route when unambiguous, required route command coverage, and local state/workstream/git evidence.
- It does not invoke Skill, run management commands, mutate settings/workspace/workstream/git/inbox/shipping/archive/surface state, ask routing questions, or perform interactive namespace management; dispatch execution is staged for namespace/Skill operation parity, so status remains adapted-safe.
- Bridge ns-manage now includes a structured Serena namespace/skill dispatch operation plan with route selection, command bundle evidence, skill-equivalent instruction modules, role workflows, checkpoints, validation, rollback, and dispatch ledger write-set evidence while preserving existing safety gates.
