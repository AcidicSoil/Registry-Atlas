---
name: gsd:ui-phase
description: "Use when operating the ui-phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge ui-phase {{phase}} --format markdown"
  variables:
    - name: "phase"
      required: false
      placeholder: "{{phase}}"
  ...
  slashAliases:
    - "/gsd:ui-phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/ui-phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-ui-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge ui-phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `ui-phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge ui-phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge ui-phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:ui-phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/ui-phase`
- Required command: `gsd-serena-bridge ui-phase --format markdown`
- Dry-run command: `gsd-serena-bridge ui-phase --format markdown`
- Apply command: `gsd-serena-bridge ui-phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/UI_PHASE.md`
- External actions:
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/ui-phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/ui-phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/ui-phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/ui-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-ui-phase`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/ui-phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/ui-phase.md`
...
- Bridge ui-phase reports UI-SPEC readiness and now mirrors write/run/agent/research modes as instruction guidance.
- Vendor UI design intent is mirrored through artifact write-set, review checklist, validation criteria, and rollback guidance using Serena role-workflow guidance instead of automatic role execution, research tools, shell commands, or UI-SPEC writes.
- Bridge ui-phase now includes a structured Serena phase-workflow operation plan with shell/test commands, phase artifact write-set, checkpoints, validation, rollback, and role workflow evidence while preserving existing safety gates.
