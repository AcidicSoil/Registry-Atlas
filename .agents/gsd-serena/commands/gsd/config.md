---
name: gsd:config
description: "Use when operating the config bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge config --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:config"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/config.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-config`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge config --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `config`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge config --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge config --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:config`
- Test fixture path: `tests/fixtures/runtime-capability/commands/config`
- Required command: `gsd-serena-bridge config --format markdown`
- Dry-run command: `gsd-serena-bridge config --format markdown`
- Apply command: `gsd-serena-bridge config --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/config.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/config.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/config.md`.
...
- Native query translated: `| --profile &lt;name&gt; | Switch model profile without interactive prompt | gsd-tools query config-set-model-profile |` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.

</routing>

<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/settings.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/settings-advanced.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/settings-integrations.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Native query translated: `2. Run: 'gsd-tools query config-set-model-profile <profile-name> --raw' and display the output verbatim.` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
...
- Contract ID: `gsd-command-config`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/config.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/config.md`
...
- Bridge config renders a reference-shaped read-only configuration report over .planning/config.json.
- The supported slice covers default/common, --advanced, and --integrations inspection with secret-value redaction.
- Profile switching is staged for explicit Serena state/release operation parity because it mutates local model profile settings.
- Bridge config remains supported for read-only inspection and now includes a Serena profile/config operation plan with redaction, config write-set, validation, and rollback.
