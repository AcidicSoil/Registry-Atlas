...
description: "Use when operating the eval-review bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge eval-review --format markdown"
  variables:
    []
  slashAliases:
    - "/gsd:eval-review"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/eval-review.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-eval-review`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge eval-review --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `eval-review`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge eval-review --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge eval-review --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:eval-review`
- Test fixture path: `tests/fixtures/runtime-capability/commands/eval-review`
- Required command: `gsd-serena-bridge eval-review --format markdown`
- Dry-run command: `gsd-serena-bridge eval-review --format markdown`
- Apply command: `gsd-serena-bridge eval-review --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
- `.planning/phases/<phase>/**`
- Write actions:
- `.planning/phases/<phase>/EVAL_REVIEW.md`
- External actions:
- none
- Git actions:
- none
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/eval-review.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/eval-review.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/eval-review.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/eval-review.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
...
- Contract ID: `gsd-command-eval-review`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/eval-review.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/eval-review.md`
...
- Bridge eval-review reports evaluation coverage evidence and now mirrors write/agent modes as instruction guidance.
- Vendor remediation intent is mirrored through AI-SPEC, dataset/eval markers, verification evidence, and draft remediation findings without automatic artifact writes or agents.
- Bridge eval-review now includes a structured Serena review/test/docs operation plan with shell validation, bounded write sets, checkpoints, rollback, and reviewer/test-writer/doc-updater role workflow evidence while preserving existing safety gates.
