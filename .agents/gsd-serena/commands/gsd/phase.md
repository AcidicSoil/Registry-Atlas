---
name: gsd:phase
description: "Use when operating the phase bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge phase {{target}} --format markdown"
  variables:
    - name: "target"
      required: false
      placeholder: "{{target}}"
      description: "Phase target, description, or phase rename expression."
  slashAliases:
    - "/gsd:phase"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/phase.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-phase`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge phase --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `phase`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge phase --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge phase --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:phase`
- Test fixture path: `tests/fixtures/runtime-capability/commands/phase`
- Required command: `gsd-serena-bridge phase --format markdown`
- Dry-run command: `gsd-serena-bridge phase --format markdown`
- Apply command: `gsd-serena-bridge phase --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/phase.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/phase.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/phase.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/add-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/insert-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/remove-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/edit-phase.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- `vendor/reference/gsd-core/commands/gsd/phase.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/phase.md`
...
- Bridge phase now supports guarded future-phase add, insert, edit, remove, and renumber operations through --write/--apply and explicit ROC phase verbs.
- Bridge phase still blocks unsafe active, completed, in-progress, evidence-bearing, or force-bypassed mutations.
- Bridge phase prepares validation-gated activation packets and includes Serena phase-workflow operation plans with shell/test commands, checkpoints, validation, rollback, and role workflow evidence.
