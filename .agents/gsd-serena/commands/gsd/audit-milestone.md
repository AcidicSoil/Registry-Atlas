...
description: "Use when operating the audit-milestone bridge command in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
promptsnippets:
  command: "gsd-serena-bridge audit-milestone {{id_or_title}} --format markdown"
  variables:
    - name: "id_or_title"
      required: false
  ...
  slashAliases:
    - "/gsd:audit-milestone"
  skillReferences:
    - ".agents/gsd-serena/commands/gsd/audit-milestone.md"
---
...
This is the bridge command Markdown artifact for `gsd-command-audit-milestone`. Vendor GSD-core commands are Markdown files; this generated file keeps that model as the first-class command artifact. Command `SKILL.md` outputs are intentionally not generated; command Markdown is the command runtime authority.
...
- Native slash command intent maps to: `gsd-serena-bridge audit-milestone --format markdown`.
...
3. Restate the user's goal in one sentence and map it to this command: `audit-milestone`.
...
7. Run or prepare the implemented bridge command from the project root:
   ...
   ```bash
   gsd-serena-bridge audit-milestone --format markdown
   ```

   If the command needs arguments, inspect `gsd-serena-bridge audit-milestone --help` or use the resolver packet instead of guessing.
...
- Contract row: `command:audit-milestone`
- Test fixture path: `tests/fixtures/runtime-capability/commands/audit-milestone`
- Required command: `gsd-serena-bridge audit-milestone --format markdown`
- Dry-run command: `gsd-serena-bridge audit-milestone --format markdown`
- Apply command: `gsd-serena-bridge audit-milestone --apply --confirm --format markdown`
- Read actions:
- `.planning/STATE.md`
- `.planning/ROADMAP.md`
...
- `pnpm vitest run tests/gsd-serena/runtime-capability/audit-milestone.test.ts`
...
Recorded path: `vendor/reference/gsd-core/commands/gsd/audit-milestone.md`; resolved path: `vendor/reference/gsd-core/commands/gsd/audit-milestone.md`.
...
<execution_context>
- GSD-core workflow import retained: `@~/.claude/gsd-core/workflows/audit-milestone.md`. Use the mirrored bridge workflow runbook under `.agents/gsd-serena/workflows/**` for adapted execution.
</execution_context>
...
- Contract ID: `gsd-command-audit-milestone`
- Family: `command`
- Status: `adapted-safe`
...
- `vendor/reference/gsd-core/commands/gsd/audit-milestone.md` (vendor-command) -> `vendor/reference/gsd-core/commands/gsd/audit-milestone.md`
...
- Supported for bridge-owned milestone metadata audit: reads .planning/.bridge/milestones.json and workstreams.json, reports scoped findings, and performs no mutation. Native Agent/integration-checker and audit-file writes remain out of scope.
