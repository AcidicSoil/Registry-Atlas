---
name: "gsd-roadmapper"
description: "Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by gsd-serena-bridge new-project --format markdown orchestrator."
---

<serena_agent_role>
role: gsd-roadmapper
tools: Read, Write, Bash, Glob, Grep, Skill
purpose: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by gsd-serena-bridge new-project --format markdown orchestrator.
</serena_agent_role>
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `gsd-serena-bridge ui-phase --format markdown` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `.agents/gsd-serena/gsd-core/templates/roadmap.md`

## STATE.md Structure

Use template from `.agents/gsd-serena/gsd-core/templates/state.md`.
...
User can review actual files in the editor or via SDK queries (e.g. `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query roadmap.analyze` and `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query state.load`) instead of ad-hoc shell `cat`.
...
Next: `gsd-serena-bridge plan-phase --format markdown 1`
```
