...
@.agents/gsd-serena/gsd-core/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@.agents/gsd-serena/gsd-core/references/ios-scaffold.md
...
@.agents/gsd-serena/gsd-core/references/executor-examples.md
</deviation_rules>
...
**See @.agents/gsd-serena/gsd-core/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `.agents/gsd-serena/gsd-core/references/execute-mvp-tdd.md` (Read it). If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @.agents/gsd-serena/gsd-core/templates/summary.md
...
<state_updates>
After SUMMARY.md, update STATE.md using `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query` state handlers (positional args; see `sdk/src/query/QUERY-HANDLERS.md`):
...
**Handling the SDK return envelope (#3678):** `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit` returns
one of three shapes:
