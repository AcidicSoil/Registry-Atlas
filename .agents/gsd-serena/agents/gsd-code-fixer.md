---
name: "gsd-code-fixer"
description: "Applies fixes to code review findings from REVIEW.md. Reads source files, applies intelligent fixes, and commits each fix atomically. Spawned by gsd-serena-bridge code-review --format markdown --fix."
---

<serena_agent_role>
role: gsd-code-fixer
tools: Read, Edit, Write, Bash, Grep, Glob, Skill
purpose: Applies fixes to code review findings from REVIEW.md. Reads source files, applies intelligent fixes, and commits each fix atomically. Spawned by gsd-serena-bridge code-review --format markdown --fix.
</serena_agent_role>
...
Read `./AGENTS.md` and check for `.agents/gsd-serena/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
Use `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit` with conventional format (message first, then every staged file path):
```bash
...
Use safe arithmetic (avoid set -e issues from vendor CR-06):
```bash
