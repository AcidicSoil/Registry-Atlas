...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.agents/gsd-serena/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
Before starting a long phase (especially `gsd-serena-bridge execute-phase --format markdown`, `gsd-serena-bridge plan-phase --format markdown`, or anything that fans out across many subagents), run this audit:
...
The keys live in `.agents/gsd-serena/settings.json` (project) or `.agents/gsd-serena/settings.json` (global) — **not** in `.planning/config.json`:
