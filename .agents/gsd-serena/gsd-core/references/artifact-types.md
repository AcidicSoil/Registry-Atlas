...
- **Location**: `.agents/gsd-serena/gsd-core/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via gsd-serena-bridge spike --format markdown)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `gsd-serena-bridge spike --format markdown` → Verified → Wrapped up by `gsd-serena-bridge spike-wrap-up --format markdown`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `gsd-serena-bridge spike-wrap-up --format markdown` for curation; `pause-work` for spike context handoff
...
- **Lifecycle**: Created by `gsd-serena-bridge sketch --format markdown` → Evaluated → Wrapped up by `gsd-serena-bridge sketch-wrap-up --format markdown`
- **Location**: `.planning/sketches/NNN-name/README.md`, `.planning/sketches/NNN-name/index.html`, `.planning/sketches/MANIFEST.md`
- **Consumed by**: `gsd-serena-bridge sketch-wrap-up --format markdown` for curation; `pause-work` for sketch context handoff

### WRAP-UP-SUMMARY.md (per wrap-up session)
- **Shape**: Curation results, included/excluded items, feature/design area groupings
- **Lifecycle**: Created by `gsd-serena-bridge spike-wrap-up --format markdown` or `gsd-serena-bridge sketch-wrap-up --format markdown`
