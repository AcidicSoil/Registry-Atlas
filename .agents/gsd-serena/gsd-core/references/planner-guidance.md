...
**Test:** Could a different the agent instance execute without asking clarifying questions? If not, add specificity. See @.agents/gsd-serena/gsd-core/references/planner-antipatterns.md for vague-vs-specific comparison table.
...
Run `/clear` first for a fresh context window, then execute: `gsd-serena-bridge execute-phase --format markdown {phase}`
```
...
Execute: `gsd-serena-bridge execute-phase --format markdown {phase} --gaps-only`
```
