...
```bash
PHASE_INFO=$(node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query find-phase "${PHASE}" --raw)
```
