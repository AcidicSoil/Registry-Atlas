...
```bash
node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
1. **Auto-detection:** During `gsd-serena-bridge new-project --format markdown`, directories with their own `.git` folder are detected and offered for selection as sub-repos. On subsequent runs, `loadConfig` auto-syncs the `sub_repos` list with the filesystem — adding newly created repos and removing deleted ones. This means `config.json` may be rewritten automatically when repos change on disk.
...
```bash
node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit-to-subrepo "feat(02-01): add user API" \
