...
1. Run `gsd-serena-bridge verify-work --format markdown` — verification finds gaps
2. Review gaps — determine which are intentional deviations
3. Add override entries to VERIFICATION.md frontmatter
4. Re-run `gsd-serena-bridge verify-work --format markdown` — overrides are applied, remaining gaps shown
...
During `gsd-serena-bridge audit-milestone --format markdown`, overrides are surfaced in the audit report:
