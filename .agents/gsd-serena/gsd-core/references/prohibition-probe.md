...
`@.agents/gsd-serena/gsd-core/references/edge-probe.md`: `edge-probe` reaches the
...
- OWASP / prototype-pollution / path-traversal / injection → breadcrumb to `gsd-serena-bridge secure-phase --format markdown`
  and `eslint` (security plugins), not a minted prohibition.
- GDPR / data-retention / consent → breadcrumb to `gsd-serena-bridge secure-phase --format markdown`.
- Generic fairness/bias canon → breadcrumb to `gsd-serena-bridge secure-phase --format markdown`.

The breadcrumb reads like: *"prototype-pollution is canon — covered by gsd-serena-bridge secure-phase --format markdown +
