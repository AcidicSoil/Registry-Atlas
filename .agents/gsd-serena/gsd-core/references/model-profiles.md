...
## Using Non-the agent Runtimes (Serena bridge reviewer, OpenCode, Gemini CLI, Kilo)
...
that support it (Serena bridge), so `model` and `reasoning_effort` always derive
...
Runtime: `gsd-serena-bridge set-profile --format markdown <profile>`
