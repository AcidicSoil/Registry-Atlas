...
- **The `gsd-roadmapper` agent makes the MVP/standard decision once at project init** based on `PROJECT_MODE`. Per-phase opt-in/out happens later via `gsd-serena-bridge mvp-phase --format markdown` or `gsd-serena-bridge edit-phase --format markdown`.
...
- `mvp-phase-command.test.cjs`, `mvp-phase-integration.test.cjs`, `mvp-phase-spidr.test.cjs` — `gsd-serena-bridge mvp-phase --format markdown`
