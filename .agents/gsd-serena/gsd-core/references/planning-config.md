...
| `manager.flags.discuss` | `""` | Flags passed to `gsd-serena-bridge discuss-phase --format markdown` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query` (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit "docs: update state" --files .planning/STATE.md

# Load config via state load (returns JSON):
INIT=$(node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# commit_docs is available in the JSON output

# Or use init commands which include commit_docs:
INIT=$(node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query init.execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for gsd-serena-bridge review --format markdown (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run gsd-serena-bridge ai-integration-phase --format markdown before planning AI system phases |
...
| `workflow.security_enforcement` | boolean | `true` | `true`, `false` | Enable threat-model-anchored security verification via `gsd-serena-bridge secure-phase --format markdown`. When `false`, security checks are skipped entirely |
...
Set via `ship.*` namespace in config.json. These fields affect `gsd-serena-bridge ship --format markdown` PRD-style pull request body composition only.
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `gsd-serena-bridge map-codebase --format markdown --query`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `gsd-serena-bridge map-codebase --format markdown --query` builds and queries a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `gsd-serena-bridge discuss-phase --format markdown` from manager (e.g., `"--auto --analyze"`) |
