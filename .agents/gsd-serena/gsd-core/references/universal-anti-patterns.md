...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/gsd-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
25. **Prefer `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`gsd-tools.cjs`** (not `gsd-tools.js` or any other filename) — GSD ships the programmatic API as CommonJS for Node.js CLI compatibility.
