...
Template for project-root `AGENTS.md` — auto-generated by `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" generate-claude-md`.
...
**Fallback text:**
```
Project not yet initialized. Run gsd-serena-bridge new-project --format markdown to set up.
```
...
**Fallback text:**
```
No project skills found. Add skills to any of: `.agents/gsd-serena/skills/`, `.agents/skills/`, `.cursor/skills/`, or `.github/skills/` with a `SKILL.md` index file.
```

**Discovery behavior:**
- Scans `.agents/gsd-serena/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/` for subdirectories containing `SKILL.md`
...
- `gsd-serena-bridge quick --format markdown` for small fixes, doc updates, and ad-hoc tasks
- `gsd-serena-bridge debug --format markdown` for investigation and bug fixing
- `gsd-serena-bridge execute-phase --format markdown` for planned phase work
...
> Profile not yet configured. Run `gsd-serena-bridge profile-user --format markdown` to generate your developer profile.
