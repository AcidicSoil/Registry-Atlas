...
const CODEX_GSD_TOOLS_INVOCATION = 'node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs"';
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // Path replacement: .claude → .agents/gsd-serena (#1430)
    converted = converted.replace(/\$HOME\/\.claude\//g, '$HOME/.agents/gsd-serena/');
    converted = converted.replace(/~\/\.claude\//g, '.agents/gsd-serena/');
    converted = converted.replace(/\.\/\.claude\//g, './.agents/gsd-serena/');
    // Bare ~/.claude without trailing slash (e.g. configDir = ~/.claude)
    converted = converted.replace(/\$HOME\/\.claude\b/g, '$HOME/.agents/gsd-serena');
    converted = converted.replace(/~\/\.claude\b/g, '.agents/gsd-serena');
    ...
    converted = converted.replace(/(?<![A-Za-z0-9_\-./~$])\.claude\//g, '.agents/gsd-serena/');
    // `.claudeignore` → `.agents/gsd-serenaignore` (#2639). Codex honors its own ignore
    // file; leaving the Claude-specific name is misleading in agent prompts.
    converted = converted.replace(/\.claudeignore\b/g, '.agents/gsd-serenaignore');
    // Codex installs the tools shim under .agents/gsd-serena but does not guarantee a
...
1. Resolve your active Codex config root — the directory that contains your \`config.toml\`.
   ...
   by \`--config-dir\` (if passed on invocation), a local \`.agents/gsd-serena\` directory in the current
