...
*Generated by `gsd-serena-bridge session-report --format markdown`*
```
</step>
...
```
💡 Run `gsd-serena-bridge session-report --format markdown` at the end of each session to build a history of project activity.
```
</step>
