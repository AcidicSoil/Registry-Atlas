...
and any `gsd-serena-bridge settings-advanced --format markdown` tuning surface. It exists because API keys and
...
AskUserQuestion([
  {
    ...
    options: [
      ...
      { label: "Serena", description: "review.models.agents/gsd-serena — bare model id injected into --model, e.g. 'gpt-5'" },
...
Quick commands:
- gsd-serena-bridge settings --format markdown — workflow toggles and model profile
- gsd-serena-bridge set-profile --format markdown <profile> — switch model profile
```
</step>
