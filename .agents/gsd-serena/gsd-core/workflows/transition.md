...
There is no `gsd-serena-bridge transition --format markdown` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `gsd-serena-bridge transition --format markdown`.
...
```text
@.agents/gsd-serena/gsd-core/workflows/graduation.md
```
...
Exit skill and invoke SlashCommand("gsd-serena-bridge plan-phase --format markdown [X+1] --auto ${GSD_WS}")
...
Exit skill and invoke SlashCommand("gsd-serena-bridge discuss-phase --format markdown [X+1] --auto ${GSD_WS}")
...
`gsd-serena-bridge discuss-phase --format markdown [X+1] ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `gsd-serena-bridge plan-phase --format markdown [X+1] ${GSD_WS}` — skip discussion, plan directly
- `gsd-serena-bridge plan-phase --format markdown --research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
`gsd-serena-bridge plan-phase --format markdown [X+1] ${GSD_WS}`

---

**Also available:**
- `gsd-serena-bridge discuss-phase --format markdown [X+1] ${GSD_WS}` — revisit context
- `gsd-serena-bridge plan-phase --format markdown --research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
`gsd-serena-bridge workstreams --format markdown complete {current_ws_name} ${GSD_WS}`

See overall milestone progress:

`gsd-serena-bridge workstreams --format markdown progress ${GSD_WS}`
...
Do NOT suggest `gsd-serena-bridge complete-milestone --format markdown` or `gsd-serena-bridge new-milestone --format markdown`.
...
Exit skill and invoke SlashCommand("gsd-serena-bridge complete-milestone --format markdown {version} ${GSD_WS}")
...
`gsd-serena-bridge complete-milestone --format markdown {version} ${GSD_WS}`
