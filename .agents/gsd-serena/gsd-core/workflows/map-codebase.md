...
variable.** Downstream steps (`agent_passes`, `sequential_mapping`, and any
...
If "Update": Ask which documents to update, continue to agent_passes (filtered)
...
→ **Skip `agent_passes` and `collect_confirmations`** — go directly to `sequential_mapping` instead.
...
<step name="agent_passes" condition="Agent tool is available">
...
<step name="sequential_mapping" condition="Agent tool is NOT available (e.g. Antigravity, Gemini CLI)">
When the `Agent` tool is unavailable, perform codebase mapping sequentially in the current context. This replaces `agent_passes` and `collect_confirmations`.
...
**Also available:**
- Re-run mapping: `gsd-serena-bridge map-codebase --format markdown`
