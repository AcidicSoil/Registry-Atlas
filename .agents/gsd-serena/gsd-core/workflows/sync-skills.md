...
**Command:** `gsd-serena-bridge sync-skills --format markdown`
...
**Supported runtime names:** `claude`, `serena`, `grok`, `copilot`, `cursor`, `windsurf`, `opencode`, `gemini`, `kilo`, `augment`, `trae`, `qwen`, `codebuddy`, `cline`, `antigravity` (grok uses the `~/.agents` layout)
...
# Parse --to
if [[ "$@" == *"--to all"* ]]; then
  TO_RUNTIMES=(claude serena grok copilot cursor windsurf opencode gemini kilo augment trae qwen codebuddy cline antigravity)
elif [[ "$@" == *"--to"* ]]; then
...
INSTALL_JS_GLOBAL=".agents/gsd-serena/gsd-core/bin/install.js"
...
error: source skills root not found: <path>
       ...
       Run: node .agents/gsd-serena/gsd-core/bin/install.js --global --<runtime>
```
Then exit.
