...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   gsd_run query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"gsd-serena-bridge plan-phase --format markdown {decimal_phase}"}'
   ```
...
- Don't use this for planned work at end of milestone (use gsd-serena-bridge add-phase --format markdown)
