...
if [ -z "$PATCHES_DIR" ] && [ -n "$GSD_SERENA_RUNTIME_ROOT" ]; then
  candidate="$(expand_home "$GSD_SERENA_RUNTIME_ROOT")/gsd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  ...
  elif [ -d "$HOME/.agents/gsd-serena/gsd-local-patches" ]; then
    PATCHES_DIR="$HOME/.agents/gsd-serena/gsd-local-patches"
  else
    PATCHES_DIR=".agents/gsd-serena/gsd-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .agents/gsd-serena .claude; do
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `.agents/gsd-serena/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
Resolve before re-running:
  ...
  (c) If the upstream changes are acceptable, update the backup-meta.json
      ...
      re-run gsd-serena-bridge update --format markdown --reapply to re-verify with the refreshed baseline.

Then re-run gsd-serena-bridge update --format markdown --reapply to re-verify.
```
...
gsd-serena-bridge update --format markdown --reapply to retry with full verification.
```
