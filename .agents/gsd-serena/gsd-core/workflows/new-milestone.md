...
- Check for MILESTONE-CONTEXT.md (from gsd-serena-bridge discuss-milestone --format markdown)
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/{FILE}
</output>
...
`gsd-serena-bridge discuss-phase --format markdown [N] ${GSD_WS}` — gather context and clarify approach

Also: `gsd-serena-bridge plan-phase --format markdown [N] ${GSD_WS}` — skip discussion, plan directly
```
...
- [ ] User knows next step: `gsd-serena-bridge discuss-phase --format markdown [N] ${GSD_WS}`
