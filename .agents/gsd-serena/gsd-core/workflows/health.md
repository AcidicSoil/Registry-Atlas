...
runtime is non-the agent (Gemini, etc.) and `AskUserQuestion` is not
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run gsd-serena-bridge health --format markdown --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run gsd-serena-bridge new-project --format markdown to create
```
...
```
---
N issues can be auto-repaired. Run: gsd-serena-bridge health --format markdown --repair
```
</step>
...
```
Would you like to run gsd-serena-bridge health --format markdown --repair to fix N issues automatically?
```
...
TASKS_DIR=".agents/gsd-serena/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in .agents/gsd-serena/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf .agents/gsd-serena/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
