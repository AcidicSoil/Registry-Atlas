...
for cand in \
  ...
  ".agents/gsd-serena/gsd-core/bin/gsd-tools.cjs"; do
...
- Line 3 = target runtime (`claude`, `opencode`, `gemini`, `kilo`, `serena`, `antigravity`)
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`.agents/gsd-serena/`, `~/.gemini/`, `.agents/gsd-serena/`, etc.), so the script invocation works for every runtime — not just the agent. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `.agents/gsd-serena/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `.agents/gsd-serena/`
local: `./.agents/gsd-serena/`, `./.config/opencode/`, `./.opencode/`, `./.gemini/`, `./.kilo/`, or `./.agents/gsd-serena/`)
...
if [ -n "$GSD_SERENA_RUNTIME_ROOT" ]; then
  CACHE_DIRS+=( "$(expand_home "$GSD_SERENA_RUNTIME_ROOT")" )
fi
...
for dir in .claude .config/opencode .opencode .gemini/antigravity-ide .gemini/antigravity-cli .gemini/antigravity .agents .agent .gemini .config/kilo .kilo .agents/gsd-serena .cursor .codeium/windsurf .augment .trae .qwen .hermes .codebuddy .cline; do
  rm -f "./$dir/cache/gsd-update-check"*.json
...
# statusline stops showing the stale "⬆ gsd-serena-bridge update --format markdown" indicator after update.
...
Run `gsd-serena-bridge update --format markdown --reapply` to merge your modifications into the new version.
```
