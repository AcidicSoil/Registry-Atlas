...
CONVERGENCE_ARGS=""
for REVIEW_FLAG in --serena-reviewer --gemini --claude --opencode --ollama --lm-studio --llama-cpp --all --text; do
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  ...
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      'gsd-serena-bridge progress --format markdown --next --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
...
Do not ask for confirmation — the whole point of `gsd-serena-bridge progress --format markdown --next` is zero-friction advancement.

**Route 3 convergence override:** When the routing decision is Route 3 (plan) and `PLAN_STRATEGY=converge`, invoke `gsd-serena-bridge plan-review-convergence --format markdown <current-phase> ${CONVERGENCE_ARGS}` instead of `gsd-serena-bridge plan-phase --format markdown <current-phase>`.

**If `--auto` was passed:** after the determined command completes, automatically re-invoke `gsd-serena-bridge progress --format markdown --next --auto` (forwarding `--converge`/`--cross-ai` and any reviewer flags if they were originally passed) to continue chaining to the next step. Repeat until one of:
- A milestone completes (`gsd-serena-bridge complete-milestone --format markdown` is reached)
...
Resume with: `gsd-serena-bridge progress --format markdown --next --auto` once resolved.
```
</step>
