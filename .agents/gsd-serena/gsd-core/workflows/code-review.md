...
No REVIEW.md created. You can retry with gsd-serena-bridge code-review --format markdown ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with gsd-serena-bridge code-review --format markdown ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  gsd-serena-bridge code-review --format markdown ${PHASE_NUMBER} --fix  — Auto-fix issues
