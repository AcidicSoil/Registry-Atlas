...
- **Global scope**: `.agents/gsd-serena/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `gsd-serena-bridge note --format markdown list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
2. Ensure the notes directory exists (`.planning/notes/` or `.agents/gsd-serena/notes/`)
...
2. Glob `.agents/gsd-serena/notes/*.md` (if directory exists) — global notes
...
Global (.agents/gsd-serena/notes/):
  4. [2026-02-08 10:00] cross-project idea about shared config

{count} active note(s). Use `gsd-serena-bridge note --format markdown promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GSD project. Run `gsd-serena-bridge new-project --format markdown` to initialize one."
...
status: pending
priority: P2
source: "promoted from gsd-serena-bridge note --format markdown"
created: {YYYY-MM-DD}
theme: general
---
...
<edge_cases>
1. **"list" as note text**: `gsd-serena-bridge note --format markdown list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `.agents/gsd-serena/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `gsd-serena-bridge new-project --format markdown`
