...
<required_reading>
@.agents/gsd-serena/gsd-core/references/domain-probes.md
@.agents/gsd-serena/gsd-core/references/gate-prompts.md
@.agents/gsd-serena/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f ".agents/gsd-serena/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
```bash
SPIKE_FINDINGS=$(ls ./.agents/gsd-serena/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.agents/gsd-serena/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
Read `@.agents/gsd-serena/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @.agents/gsd-serena/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```
Apply each entry in `activeHooks` per @.agents/gsd-serena/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`gsd-serena-bridge plan-phase --format markdown ${PHASE} ${GSD_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `gsd-serena-bridge plan-phase --format markdown ${PHASE} --skip-research ${GSD_WS}` to plan without research; `gsd-serena-bridge ui-phase --format markdown ${PHASE} ${GSD_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
