...
<step name="sequential_generation" condition="Task tool is NOT available (e.g. Antigravity, Gemini CLI, Copilot)">
...
Display note:
```
To fix failures automatically: gsd-serena-bridge docs-update --format markdown (runs generation + fix loop)
To regenerate all docs from scratch: gsd-serena-bridge docs-update --format markdown --force
```
...
3. Re-run gsd-serena-bridge docs-update --format markdown to regenerate clean docs
```
...
```
Run `gsd-serena-bridge docs-update --format markdown --verify-only` to fact-check generated docs against the codebase.
```
