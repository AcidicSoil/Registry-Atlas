...
Saves artifacts to `.planning/spikes/`. Companion to `gsd-serena-bridge spike --format markdown --wrap-up`.
...
**b. Findings skills** — glob `./.agents/gsd-serena/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md`. These contain curated knowledge from prior wrap-ups.
...
**b. Findings skills:** Glob for `./.agents/gsd-serena/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md` files.
...
**Also available:**
- `gsd-serena-bridge spike --format markdown` — spike more ideas (or run with no argument for frontier mode)
- `gsd-serena-bridge plan-phase --format markdown` — start planning the real implementation
- `gsd-serena-bridge explore --format markdown` — continue exploring the idea
