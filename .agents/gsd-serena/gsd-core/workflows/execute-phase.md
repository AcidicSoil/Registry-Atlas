...
@.agents/gsd-serena/gsd-core/references/context-budget.md
@.agents/gsd-serena/gsd-core/references/gates.md
</required_reading>

<available_agent_types>
These are the valid GSD subagent types registered in .agents/gsd-serena/agents/ (or equivalent for your runtime).
...
if [ "$RUNTIME" = "serena" ] && [ "$USE_WORKTREES" != "false" ]; then
  echo "FATAL: Serena bridge execute-phase worktree isolation is unsupported. Set workflow.use_worktrees=false or use a runtime with Agent isolation=\"worktree\" support." >&2
  exit 1
fi
...
```
Serena bridge maps subagents to `agent_pass`, which has no direct Serena bridge mapping for Claude Code's `isolation="worktree"` parameter. Failing closed prevents main-checkout edits while the workflow believes agents are isolated.
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @.agents/gsd-serena/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @.agents/gsd-serena/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `.agents/gsd-serena/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `.agents/gsd-serena/gsd-core/workflows/execute-plan.md` **inline**
...
per-agent branch namespace `worktree-agent-*`, NOT the `.agents/gsd-serena/worktrees/` path: the
orchestrator may itself be legitimately invoked from a feature worktree under
`.agents/gsd-serena/worktrees/`, so a path-substring refusal would break legitimate runs. Do NOT
...
tooling and `gsd-serena-bridge manager --format markdown`'s background-completion handler can grep partial
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       <execution_context>
       @.agents/gsd-serena/gsd-core/workflows/execute-plan.md
       @.agents/gsd-serena/gsd-core/templates/summary.md
       @.agents/gsd-serena/gsd-core/references/checkpoints.md
       @.agents/gsd-serena/gsd-core/references/tdd.md
       @.agents/gsd-serena/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@.agents/gsd-serena/gsd-core/references/executor-examples.md'}
       </execution_context>
...
7. **Handle failures:**
   **Step 7.0 — classify before branching (#3095):**
   ...
   ```
   One classifier branch handles sentinels across the agent/Copilot/Gemini. Reference: `docs/research/provider-rate-limit-signals.md`.
   **Step 7.1 — `class == "quota-exceeded"`:**
   ...
   ```
   Re-run `gsd-serena-bridge execute-phase --format markdown` after quota reset for Option 1.
...
⚠ Security enforcement enabled — run before advancing:
  gsd-serena-bridge secure-phase --format markdown {PHASE} ${GSD_WS}
```

If an active secure-phase step hook exists AND SECURITY.md exists: check frontmatter `threats_open`. If > 0:
```
⚠ Security gate: {threats_open} threats open
  gsd-serena-bridge secure-phase --format markdown {PHASE} — resolve before advancing
```
</step>
...
gsd-serena-bridge execute-phase --format markdown {phase} ${GSD_WS}                # Continue remaining waves
gsd-serena-bridge execute-phase --format markdown {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
gsd-serena-bridge code-review --format markdown ${PHASE_NUMBER} --fix
```
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `.agents/gsd-serena/gsd-core/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:
...
`gsd-serena-bridge verify-work --format markdown {X} ${GSD_WS}`
...
**If user acknowledges without reporting issues (including "ok", "noted", "ack", "got it", "approved", "done", "yes", "pass", or similar):** Stop. The phase remains pending. No further orchestrator action — wait for the user to run `gsd-serena-bridge verify-work --format markdown`.
...
`gsd-serena-bridge plan-phase --format markdown {X} --gaps ${GSD_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `gsd-serena-bridge verify-work --format markdown {X} ${GSD_WS}` — manual testing first
```

Gap closure cycle: `gsd-serena-bridge plan-phase --format markdown {X} --gaps ${GSD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `gsd-serena-bridge execute-phase --format markdown {X} --gaps-only ${GSD_WS}` → verifier re-runs.
</step>
...
These items are tracked and will appear in `gsd-serena-bridge progress --format markdown` and `gsd-serena-bridge audit-uat --format markdown`.
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`gsd-serena-bridge plan-phase --format markdown {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `.agents/gsd-serena/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `gsd-serena-bridge transition --format markdown` command. Never suggest it. The transition workflow is internal only.**
...
gsd-serena-bridge progress --format markdown ${GSD_WS} — see updated roadmap
gsd-serena-bridge discuss-phase --format markdown {next} ${GSD_WS} — start here: discuss next phase before planning  ← recommended
gsd-serena-bridge plan-phase --format markdown {next} ${GSD_WS} — plan next phase (skip discuss)
gsd-serena-bridge execute-phase --format markdown {next} ${GSD_WS} — execute next phase (skip discuss and plan)
```
...
gsd-serena-bridge progress --format markdown ${GSD_WS} — see updated roadmap
gsd-serena-bridge plan-phase --format markdown {next} ${GSD_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
gsd-serena-bridge discuss-phase --format markdown {next} ${GSD_WS} — re-discuss next phase
gsd-serena-bridge execute-phase --format markdown {next} ${GSD_WS} — execute next phase (skip planning)
```
...
<resumption>
Re-run `gsd-serena-bridge execute-phase --format markdown {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
