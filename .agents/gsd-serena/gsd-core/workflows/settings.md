...
tiers resolve to runtime-native model IDs — on Serena bridge that's gpt-5.5 / gpt-5.4 /
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Adaptive (Recommended)", description: "Role-based cost optimization: heavy roles use the highest-tier model available on the active runtime, light roles use the cheapest. Best balance of quality and cost across all supported runtimes (the agent, Gemini, OpenRouter, local)." },
      { label: "Standard tier…", description: "Choose Quality, Balanced, or Budget — flat tier applied to all agents" },
      { label: "Inherit", description: "Use current session model for all agents (required for non-the agent runtimes: Gemini CLI, OpenRouter, local models)" }
    ]
  }
])
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run gsd-serena-bridge ui-phase --format markdown before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run gsd-serena-bridge ui-phase --format markdown first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run gsd-serena-bridge ai-integration-phase --format markdown before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in gsd-serena-bridge autonomous --format markdown — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via gsd-serena-bridge map-codebase --format markdown --query — builds a JSON index in .planning/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable gsd-serena-bridge map-codebase --format markdown --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via gsd-serena-bridge graphify --format markdown — builds a graph in .planning/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable gsd-serena-bridge graphify --format markdown commands. Builds and queries a project knowledge graph." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Manual gsd-serena-bridge graphify --format markdown build only. Conservative default — opt in if you want fresh context on every gsd-serena-bridge quick --format markdown or gsd-serena-bridge plan-phase --format markdown." },
...
```json
{
  ...
  "model_policy": {
    // Read-only in this flow — written only by gsd-serena-bridge config --format markdown --advanced (Section 8).
...
These settings apply to future gsd-serena-bridge plan-phase --format markdown and gsd-serena-bridge execute-phase --format markdown runs.

Quick commands:
- gsd-serena-bridge config --format markdown --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- gsd-serena-bridge config --format markdown --profile <profile> — switch model profile
- gsd-serena-bridge plan-phase --format markdown --research — force research
- gsd-serena-bridge plan-phase --format markdown --skip-research — skip research
- gsd-serena-bridge plan-phase --format markdown --skip-verify — skip plan check
- gsd-serena-bridge config --format markdown --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window, model policy)
```
</step>
