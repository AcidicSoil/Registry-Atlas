...
Run this command from a directory containing git repos, or specify repos explicitly:
  gsd-serena-bridge workspace --format markdown --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  gsd-serena-bridge workspace --format markdown --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  gsd-serena-bridge new-project --format markdown    # Initialize GSD in the workspace
```
...
- options:
  - "Yes — run gsd-serena-bridge new-project --format markdown" → tell user to `cd "$TARGET_PATH"` first, then run `gsd-serena-bridge new-project --format markdown`
