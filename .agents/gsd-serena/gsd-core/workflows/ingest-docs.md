...
- execution_context path `/.agents/gsd-serena/` → `RUNTIME=serena`
- `/.gemini/` → `RUNTIME=gemini`
- `/.opencode/` or `/.config/opencode/` → `RUNTIME=opencode`
- else → `RUNTIME=claude`

Fall back to env vars (`GSD_SERENA_RUNTIME_ROOT`, `GEMINI_CONFIG_DIR`, `OPENCODE_CONFIG_DIR`) if execution_context is unavailable.
...
- Next step: `gsd-serena-bridge plan-phase --format markdown 1` (new mode) or `gsd-serena-bridge plan-phase --format markdown N` (merge, pointing at the first newly-added phase)
