...
@.agents/gsd-serena/gsd-core/references/git-integration.md
</required_reading>
...
See `.agents/gsd-serena/gsd-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See .agents/gsd-serena/gsd-core/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.agents/gsd-serena/gsd-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.agents/gsd-serena/gsd-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.agents/gsd-serena/gsd-core/templates/summary.md`.
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY — skip any plan whose `plan_id` matches a non-terminal async-job manifest (`external_job_waiting`; see `identify_plan`). Yolo: auto-continue. Interactive: show next plan, suggest `gsd-serena-bridge execute-phase --format markdown {phase}` + `gsd-serena-bridge verify-work --format markdown`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `gsd-serena-bridge plan-phase --format markdown {Z+1}` + `gsd-serena-bridge verify-work --format markdown {Z}` + `gsd-serena-bridge discuss-phase --format markdown {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `gsd-serena-bridge complete-milestone --format markdown` + `gsd-serena-bridge verify-work --format markdown` + `gsd-serena-bridge add-phase --format markdown` |
