...
{if bugs discovered:}
**Fix discovered bugs:** `gsd-serena-bridge quick --format markdown fix the {N} test failures discovered in phase ${phase_number}`
...
**Also available:**
- `gsd-serena-bridge add-tests --format markdown {next_phase}` — test another phase
- `gsd-serena-bridge verify-work --format markdown {phase_number}` — run UAT verification
