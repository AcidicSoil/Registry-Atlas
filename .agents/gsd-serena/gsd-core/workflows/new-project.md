...
- Path contains `/.agents/gsd-serena/` → `RUNTIME=serena`
...
```bash
if [ -n "$GSD_SERENA_RUNTIME_ROOT" ]; then RUNTIME="serena"
...
```bash
if [ "$RUNTIME" = "serena" ]; then INSTRUCTION_FILE="AGENTS.md"; else INSTRUCTION_FILE=".agents/gsd-serena/AGENTS.md"; fi
```
...
SPIKE_SKILL=$(ls ./.agents/gsd-serena/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)

# Check for sketch findings skill (project-local)
SKETCH_SKILL=$(ls ./.agents/gsd-serena/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .planning/spikes/ — consider `gsd-serena-bridge spike --format markdown --wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .planning/sketches/ — consider `gsd-serena-bridge sketch --format markdown --wrap-up` to package findings
...
**After each phase transition** (via `gsd-serena-bridge transition --format markdown`):
...
**PR body onboarding:** Ask which optional PRD-style sections `gsd-serena-bridge ship --format markdown` should append to generated PR bodies. Use the same `ship.pr_body_sections` mapping as Step 2a: selected sections get `enabled: true`, seeded-but-unselected sections get `enabled: false`, and selecting none writes an empty list. Prefer lean/agile PRD sections that make user value, acceptance criteria, Definition of Done, and stakeholder traceability explicit.
...
**Note:** Run `gsd-serena-bridge settings --format markdown` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/STACK.md
</output>
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/FEATURES.md
</output>
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/ARCHITECTURE.md
</output>
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/PITFALLS.md
</output>
...
Use template: .agents/gsd-serena/gsd-core/templates/research-project/SUMMARY.md
...
1. Verify `.planning/research/SUMMARY.md` exists AND is substantive — non-empty, and free of any leftover `<!-- gsd:write-continue -->` continuation sentinel (which marks a truncated/incomplete write). You may validate with `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" verify-summary .planning/research/SUMMARY.md` — it exits 0 regardless, so check its JSON `passed` field (`"passed": false` means missing or invalid), not the process exit code. If it passes, continue normally.
2. If it is MISSING or invalid AND the synthesizer's return message contains the FULL SUMMARY.md document — recognizable by the template's top-level markers `# Project Research Summary`, `## Key Findings`, `## Implications for Roadmap`, and `## Sources`, not merely the brief `## SYNTHESIS COMPLETE` confirmation — the false-refusal fired: write that returned document to `.planning/research/SUMMARY.md` with the Write tool, then commit ALL research artifacts the synthesizer owns (it commits on behalf of the four researchers) with `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query commit "docs: complete project research" --files .planning/research/` unless they are already committed. Log `⚠ #222 self-heal: synthesizer returned SUMMARY.md inline without writing it; orchestrator persisted the file.`
...
Exit skill and invoke SlashCommand("gsd-serena-bridge discuss-phase --format markdown 1 --auto")
...
gsd-serena-bridge discuss-phase --format markdown 1 — gather context and clarify approach

---

**Also available:**
- gsd-serena-bridge ui-phase --format markdown 1 — generate UI design contract (recommended for frontend phases)
- gsd-serena-bridge plan-phase --format markdown 1 — skip discussion, plan directly
...
- `.planning/ROADMAP.md`
- `.planning/STATE.md`
- `$INSTRUCTION_FILE` (`AGENTS.md` for Serena bridge, `.agents/gsd-serena/AGENTS.md` for all other runtimes)
...
- [ ] `$INSTRUCTION_FILE` generated with GSD workflow guidance (AGENTS.md for Serena bridge, `.agents/gsd-serena/AGENTS.md` otherwise; an existing hand-crafted file without GSD markers is left untouched unless `--force`)
- [ ] User knows next step is `gsd-serena-bridge discuss-phase --format markdown 1`
