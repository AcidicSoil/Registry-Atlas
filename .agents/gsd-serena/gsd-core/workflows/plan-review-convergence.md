...
gsd-plan-phase N → gsd-review N --serena-reviewer → gsd-plan-phase N --reviews → gsd-review N --serena-reviewer → ...
...
Extract from {{GSD_ARGS}}: phase number, reviewer flags (`--serena-reviewer`, `--gemini`, `--claude`, `--opencode`, `--ollama`, `--lm-studio`, `--llama-cpp`, `--all`), `--max-cycles N`, `--text`, `--ws`.

```bash
PHASE=$(echo "{{GSD_ARGS}}" | grep -oE '[0-9]+\.?[0-9]*' | head -1)

REVIEWER_FLAGS=""
echo "{{GSD_ARGS}}" | grep -q '\-\-serena-reviewer' && REVIEWER_FLAGS="$REVIEWER_FLAGS --serena-reviewer"
...
if [ -z "$REVIEWER_FLAGS" ]; then REVIEWER_FLAGS="--serena-reviewer"; fi
...
To replan manually:  gsd-serena-bridge plan-phase --format markdown {PHASE} --reviews
To restart loop:     gsd-serena-bridge plan-review-convergence --format markdown {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
