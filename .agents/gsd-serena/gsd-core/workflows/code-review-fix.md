...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: gsd-serena-bridge code-review --format markdown ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  gsd-serena-bridge verify-work --format markdown  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  gsd-serena-bridge code-review --format markdown ${PHASE_NUMBER}           — Re-review code"
  echo "  gsd-serena-bridge verify-work --format markdown                           — Verify phase completion"
  echo ""
fi
```
