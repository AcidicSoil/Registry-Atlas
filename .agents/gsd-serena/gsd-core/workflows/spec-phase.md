...
ones). Reference: @.agents/gsd-serena/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / .agents/gsd-serena.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  ".agents/gsd-serena/gsd-core/bin/lib/edge-probe.cjs" \
  ".agents/gsd-serena/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @.agents/gsd-serena/gsd-core/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   gsd-serena-bridge secure-phase --format markdown + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   gsd-serena-bridge secure-phase --format markdown and drowns the bespoke signal.
...
Use the SPEC.md template from @.agents/gsd-serena/gsd-core/templates/spec.md.
...
Next: gsd-serena-bridge discuss-phase --format markdown {X}
...
- User directed to gsd-serena-bridge discuss-phase --format markdown as next step
