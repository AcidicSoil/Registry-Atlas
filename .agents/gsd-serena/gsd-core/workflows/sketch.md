...
**b. Findings skills** — glob `./.agents/gsd-serena/skills/sketch-findings-*/SKILL.md` and read any that exist, plus their `references/*.md`. These contain curated design decisions from prior wrap-ups.
...
**a.** Glob for `./.agents/gsd-serena/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md`. These contain validated patterns and requirements.
...
**Also available:**
- `gsd-serena-bridge sketch --format markdown` — sketch more (or run with no argument for frontier mode)
- `gsd-serena-bridge plan-phase --format markdown` — start building the real UI
- `gsd-serena-bridge spike --format markdown` — spike technical feasibility of a design pattern
