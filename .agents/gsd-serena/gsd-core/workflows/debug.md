...
Invoked by `gsd-serena-bridge debug --format markdown` (`commands/gsd/debug.md`).
...
─────────────────────────────────────────────
Run `gsd-serena-bridge debug --format markdown continue <slug>` to resume a session.
No sessions? `gsd-serena-bridge debug --format markdown <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `gsd-serena-bridge debug --format markdown <issue description>` to start one."
...
**Sanitize SLUG first:** strip whitespace, reject unless it matches `^[a-z0-9][a-z0-9-]*$`, enforce max 30 chars, reject any `..`, `/`, or `\`. If invalid, print "No active debug session found with slug: {SLUG}. Check `gsd-serena-bridge debug --format markdown list` for active sessions." and stop.

Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `gsd-serena-bridge debug --format markdown list` for active sessions." and stop.
...
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `gsd-serena-bridge debug --format markdown continue {slug}`.
