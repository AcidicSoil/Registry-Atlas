...
<template>
@.agents/gsd-serena/gsd-core/templates/UAT.md
</template>
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@.agents/gsd-serena/gsd-core/references/verify-mvp-mode.md`. Briefly:
...
```
⚠ Security enforcement enabled — gsd-serena-bridge secure-phase --format markdown {phase} did not produce SECURITY.md.
...
- `gsd-serena-bridge secure-phase --format markdown {phase}` — security review (required before advancing)
- `gsd-serena-bridge plan-phase --format markdown {next}` — Plan next phase
- `gsd-serena-bridge execute-phase --format markdown {next}` — Execute next phase
- `gsd-serena-bridge ui-review --format markdown {phase}` — visual quality audit (if frontend files were modified)
```
...
Read and follow `.agents/gsd-serena/gsd-core/workflows/transition.md`.
...
- `gsd-serena-bridge plan-phase --format markdown {next}` — Plan next phase
- `gsd-serena-bridge execute-phase --format markdown {next}` — Execute next phase
- `gsd-serena-bridge secure-phase --format markdown {phase}` — security review
- `gsd-serena-bridge ui-review --format markdown {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
If user declines: stop. User resolves items and re-runs `gsd-serena-bridge verify-work --format markdown`.
...
- Follow @.agents/gsd-serena/gsd-core/workflows/diagnose-issues.md
...
<downstream_consumer>
Output consumed by gsd-serena-bridge execute-phase --format markdown
...
3. Abandon (exit, user runs gsd-serena-bridge plan-phase --format markdown manually)
...
`gsd-serena-bridge execute-phase --format markdown {phase} --gaps-only`
...
- [ ] Ready for `gsd-serena-bridge execute-phase --format markdown --gaps-only` when complete
</success_criteria>
