...
@.agents/gsd-serena/gsd-core/references/questioning.md
...
| Spike | `gsd-serena-bridge spike --format markdown` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `gsd-serena-bridge sketch --format markdown` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `gsd-serena-bridge add-phase --format markdown` command via SlashCommand
...
Continue exploring with `gsd-serena-bridge explore --format markdown` or start working with `gsd-serena-bridge progress --format markdown --next`.
```
