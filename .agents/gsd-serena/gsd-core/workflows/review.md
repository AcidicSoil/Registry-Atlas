...
command -v serena >/dev/null 2>&1 && echo "serena-reviewer:available" || echo "serena-reviewer:missing"
...
- `--serena-reviewer` → include Serena bridge
...
1. Individual reviewer flags (`--gemini`, `--serena-reviewer`, etc.)
2. `--all`
3. `review.default_reviewers`
4. No key + no flags → all detected reviewers

`review.default_reviewers` behavior:
- Value must be a non-empty array of slug strings (configured via `gsd config-set review.default_reviewers '["gemini","serena-reviewer"]'`)
...
- serena: https://github.com/openai/serena
...
if [ "$ANTIGRAVITY_AGENT" = "1" ]; then
  ...
  # Other environments (Gemini CLI, Serena bridge reviewer, etc.)
...
- If `SELF_CLI="claude"` → skip claude, use gemini/serena-reviewer
...
SERENA_REVIEWER_MODEL=$(gsd_run query config-get review.models.agents/gsd-serena 2>/dev/null | jq -r '.' 2>/dev/null || true)
...
# #1115: `--dangerously-bypass-hook-trust` only exists on serena-bridge >= 0.137.0.
# Capability-probe it so older installs don't fail with "unexpected argument"
# (which, with stderr suppressed, produced a silent empty review). The serena
# invocation works fine without the flag on older versions.
if gsd-serena-bridge review --format markdown --help 2>/dev/null | grep -q -- '--dangerously-bypass-hook-trust'; then
  SERENA_REVIEWER_BYPASS_FLAG="--dangerously-bypass-hook-trust"
else
  SERENA_REVIEWER_BYPASS_FLAG=""
fi
```
...
**Serena bridge:**
```bash
# $SERENA_REVIEWER_BYPASS_FLAG is capability-gated above (#1115). Capture stderr to a .err
# file (not /dev/null) so a non-zero exit — e.g. a flag the installed serena-bridge
# does not support — is diagnosable instead of a silent empty review.
if [ -n "$SERENA_REVIEWER_MODEL" ] && [ "$SERENA_REVIEWER_MODEL" != "null" ]; then
  cat /tmp/gsd-review-prompt-{phase}.md | serena exec --ephemeral $SERENA_REVIEWER_BYPASS_FLAG --model "$SERENA_REVIEWER_MODEL" --skip-git-repo-check - 2>/tmp/gsd-review-serena-{phase}.err > /tmp/gsd-review-serena-{phase}.md
else
  cat /tmp/gsd-review-prompt-{phase}.md | serena exec --ephemeral $SERENA_REVIEWER_BYPASS_FLAG --skip-git-repo-check - 2>/tmp/gsd-review-serena-{phase}.err > /tmp/gsd-review-serena-{phase}.md
fi
if [ ! -s /tmp/gsd-review-serena-{phase}.md ]; then
  echo "Serena bridge review failed or returned empty output. stderr:" > /tmp/gsd-review-serena-{phase}.md
  cat /tmp/gsd-review-serena-{phase}.err >> /tmp/gsd-review-serena-{phase}.md
fi
```
...
---
phase: {N}
reviewers: [gemini, claude, serena-reviewer, coderabbit, opencode, qwen, cursor, antigravity, ollama, lm_studio, llama_cpp]  # populate at runtime with only the reviewers actually invoked
...
## Serena bridge Review

{serena-reviewer review content}
...
To incorporate feedback into planning:
  gsd-serena-bridge plan-phase --format markdown {N} --reviews
```
...
- [ ] User knows how to use feedback (gsd-serena-bridge plan-phase --format markdown --reviews)
</success_criteria>
