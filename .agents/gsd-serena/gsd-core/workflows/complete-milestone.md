...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `gsd-serena-bridge cleanup --format markdown` later to archive retroactively.
...
**If doesn't exist:** Create from template at `.agents/gsd-serena/gsd-core/templates/retrospective.md`.
...
`gsd-serena-bridge new-milestone --format markdown`
...
- [ ] User knows next step (gsd-serena-bridge new-milestone --format markdown)
