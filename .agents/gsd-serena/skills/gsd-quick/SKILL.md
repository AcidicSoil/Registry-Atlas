...
- Do not invoke vendor-native skill names from this surface. Use the bridge command when it exists: `gsd-serena-bridge quick --format markdown`.
...
<execution_context>
@.agents/gsd-serena/gsd-core/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
5. Load context via:
   ```bash
   node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query init.quick
   ```
...
- Status fields read via `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
