...
- Do not invoke vendor-native skill names from this surface. Use the bridge command when it exists: `gsd-serena-bridge mempalace-recall --format markdown`.
...
**DO NOT use `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" config get-value`** -- it hard-exits on missing keys.
