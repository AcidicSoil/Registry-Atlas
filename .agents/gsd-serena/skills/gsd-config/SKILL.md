...
- Do not invoke vendor-native skill names from this surface. Use the bridge command when it exists: `gsd-serena-bridge config --format markdown`.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query config-set-model-profile |

</routing>

<execution_context>
@.agents/gsd-serena/gsd-core/workflows/settings.md
@.agents/gsd-serena/gsd-core/workflows/settings-advanced.md
@.agents/gsd-serena/gsd-core/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  ...
  2. Run: `node "$HOME/.agents/gsd-serena/gsd-core/bin/gsd-tools.cjs" query config-set-model-profile <profile-name> --raw` and display the output verbatim.
