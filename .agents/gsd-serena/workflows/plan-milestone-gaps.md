# Bridge Workflow: plan-milestone-gaps

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-plan-milestone-gaps` in a target project.
...
- Contract ID: `gsd-workflow-plan-milestone-gaps`
- Status: `planned`
- Source path: `gsd-core/workflows/plan-milestone-gaps.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/plan-milestone-gaps.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/plan-milestone-gaps.md`
...
<purpose>
Create all phases necessary to close gaps identified by ``gsd-serena-bridge audit-milestone --format markdown``. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gsd-add-phase` per gap.
</purpose>
...
- Native query translated: `HIGHEST=$(gsd_run query phases.list --pick directories[-1])` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
...
```bash
- Native query translated: `INIT=$(gsd_run query init.phase-op "{NN}")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
