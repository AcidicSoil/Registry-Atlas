# Bridge Workflow: ai-integration-phase

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-ai-integration-phase` in a target project.
...
- Contract ID: `gsd-workflow-ai-integration-phase`
- Status: `planned`
- Source path: `gsd-core/workflows/ai-integration-phase.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/ai-integration-phase.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/ai-integration-phase.md`
...
<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates gsd-framework-selector → gsd-ai-researcher → gsd-domain-researcher → gsd-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase ...
...
```bash
- Native query translated: `SELECTOR_MODEL=$(gsd_run query resolve-model gsd-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `RESEARCHER_MODEL=$(gsd_run query resolve-model gsd-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `DOMAIN_MODEL=$(gsd_run query resolve-model gsd-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `PLANNER_MODEL=$(gsd_run query resolve-model gsd-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```

Check config:
```bash
- Native query translated: `AI_PHASE_ENABLED=$(gsd_run query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via `gsd-serena-bridge settings --format markdown`.
```
Exit workflow.
...
Recommended: run `gsd-serena-bridge discuss-phase --format markdown` {N} first to capture framework preferences.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run `gsd-serena-bridge ai-integration-phase --format markdown` {N} or answer the framework question in `gsd-serena-bridge discuss-phase --format markdown` {N} first."
...
> **Ordering note (prevents tool-level last-writer-wins race):** Steps 7 and 8 write disjoint sections of AI-SPEC.md but MUST run sequentially — wait for Step 7 to complete before spawning Step 8. Both agents use the `Edit` tool exclusively (never `Write`) ...
...
Next step:
`gsd-serena-bridge plan-phase --format markdown` {N}   — planner will consume AI-SPEC.md
```
