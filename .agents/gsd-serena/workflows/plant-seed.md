# Bridge Workflow: plant-seed

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-plant-seed` in a target project.
...
- Contract ID: `gsd-workflow-plant-seed`
- Status: `planned`
- Source path: `gsd-core/workflows/plant-seed.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/plant-seed.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/plant-seed.md`
...
- `$ENRICH_TARGET` and `$SEED_FILE` are already set by `parse-idea`. Derive `$SEED_ID` from `$ENRICH_TARGET` (e.g. `SEED_ID="$ENRICH_TARGET"`). If `$SEED_FILE` is empty, fall back to the most-recently modified file in `.planning/seeds/` and set `$SEED_ID` f...
