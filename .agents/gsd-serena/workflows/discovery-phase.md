# Bridge Workflow: discovery-phase

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-discovery-phase` in a target project.
...
- Contract ID: `gsd-workflow-discovery-phase`
- Status: `planned`
- Source path: `gsd-core/workflows/discovery-phase.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/discovery-phase.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/discovery-phase.md`
...
NOTE: For comprehensive ecosystem research ("how do experts build this"), use `gsd-serena-bridge plan-phase --format markdown` --research-phase instead, which produces RESEARCH.md.
</purpose>
...
```
- Native MCP/tool seam translated: `mcp__context7__resolve-library-id with libraryName: "[library]"` -> use the available Serena/web/tool equivalent only when available; otherwise record an operation-plan fallback.
```

2. Fetch relevant docs:

```
- Native MCP/tool seam translated: `mcp__context7__get-library-docs with:` -> use the available Serena/web/tool equivalent only when available; otherwise record an operation-plan fallback.
...
- Native MCP/tool seam translated: `- mcp__context7__resolve-library-id` -> use the available Serena/web/tool equivalent only when available; otherwise record an operation-plan fallback.
- Native MCP/tool seam translated: `- mcp__context7__get-library-docs (mode: "code" for API, "info" for concepts)` -> use the available Serena/web/tool equivalent only when available; otherwise record an operation-plan fallback.
```
...
1. Discuss phase context (`gsd-serena-bridge discuss-phase --format markdown` [current-phase])
2. Create phase plan (`gsd-serena-bridge plan-phase --format markdown` [current-phase])
