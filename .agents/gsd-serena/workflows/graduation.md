# Bridge Workflow: graduation

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-graduation` in a target project.
...
- Contract ID: `gsd-workflow-graduation`
- Status: `planned`
- Source path: `gsd-core/workflows/graduation.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/graduation.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/graduation.md`
...
```bash
- Native query translated: `GRADUATION_ENABLED=$(gsd_run query config-get features.graduation 2>/dev/null || echo "true")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `GRADUATION_WINDOW=$(gsd_run query config-get features.graduation_window 2>/dev/null || echo "5")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `GRADUATION_THRESHOLD=$(gsd_run query config-get features.graduation_threshold 2>/dev/null || echo "3")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
...
On the first transition after upgrading to a version that includes this workflow, all extant LEARNINGS.md files may produce a large batch of candidates at once. A `[Defer all]` shorthand is available: if the developer enters `A` at any cluster prompt, all r...
