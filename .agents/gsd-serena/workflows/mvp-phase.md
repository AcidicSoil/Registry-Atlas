# Bridge Workflow: mvp-phase

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-mvp-phase` in a target project.
...
- Contract ID: `gsd-workflow-mvp-phase`
- Status: `planned`
- Source path: `gsd-core/workflows/mvp-phase.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/mvp-phase.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/mvp-phase.md`
...
<purpose>
Guide the user through MVP-mode planning for a phase. Prompts for an "As a / I want to / So that" user story, runs SPIDR splitting check on the story, writes the result to ROADMAP.md, and delegates to `/gsd plan-phase` (which auto-detects MVP via the roadma...
</purpose>
...
```bash
- Native query translated: `USER_STORY_RESULT=$(gsd_run query user-story.validate --story "$USER_STORY")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
...
- **Accept**: `USER_STORY` becomes the first split's story (`${HAPPY_STORY}` in the example). Surface the remaining splits as a list of `/gsd add-phase` invocations the user can run after this command completes — do NOT auto-create the new phases (preserve ...
...
```bash
- Native query translated: `NEW_MODE=$(gsd_run query roadmap.get-phase "${PHASE}" --pick mode)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
- Native query translated: `NEW_GOAL=$(gsd_run query roadmap.get-phase "${PHASE}" --pick goal)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
