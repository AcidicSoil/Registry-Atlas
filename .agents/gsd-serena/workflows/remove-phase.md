# Bridge Workflow: remove-phase

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-remove-phase` in a target project.
...
- Contract ID: `gsd-workflow-remove-phase`
- Status: `planned`
- Source path: `gsd-core/workflows/remove-phase.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/remove-phase.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/remove-phase.md`
...
```bash
- Native query translated: `RESULT=$(gsd_run query phase.remove "${target}")` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```

If the phase has executed plans (SUMMARY.md files), the CLI will error. Use `--force` only if the user confirms:

```bash
- Native query translated: `RESULT=$(gsd_run query phase.remove "${target}" --force)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
