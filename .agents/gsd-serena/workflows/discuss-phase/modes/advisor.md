# Bridge Workflow: discuss-phase-modes-advisor

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-discuss-phase-modes-advisor` in a target project.
...
- Contract ID: `gsd-workflow-discuss-phase-modes-advisor`
- Status: `planned`
- Source path: `gsd-core/workflows/discuss-phase/modes/advisor.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/discuss-phase/modes/advisor.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/discuss-phase/modes/advisor.md`
...
```bash
- Native query translated: `ADVISOR_MODEL=$(gsd_run query resolve-model gsd-advisor-researcher --raw)` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
...
- Native agent dispatch translated: `2. For EACH user-selected gray area, spawn a 'Agent()' in parallel:` -> use Serena role workflow / generated role skill / sequential role pass with handoff.
...
- Native agent dispatch translated: `All 'Agent()' calls spawn simultaneously — do NOT wait for one before` -> use Serena role workflow / generated role skill / sequential role pass with handoff.
starting the next.

- Native agent dispatch translated: `> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling all Agent() calls above to spawn research agents, do NOT independently research or analyze any of the gray areas while the subagents are active. Wait for all subagents to return before synthesizing results. This prevents duplicate work and wasted context.` -> use Serena role workflow / generated role skill / sequential role pass with handoff.
...
f. **If `NON_TECHNICAL_OWNER` is true:** apply a plain language rewrite to the rationale paragraph. Replace implementation-level terms with outcome descriptions the user can reason about without technical context. The Recommendation column value and the tab...
