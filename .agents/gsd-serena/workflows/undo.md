# Bridge Workflow: undo

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-undo` in a target project.
...
- Contract ID: `gsd-workflow-undo`
- Status: `planned`
- Source path: `gsd-core/workflows/undo.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/undo.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/undo.md`
...
```
Usage: `gsd-serena-bridge undo --format markdown` --last N | --phase NN | --plan NN-MM
...
Examples:
`gsd-serena-bridge undo --format markdown` --last 5
`gsd-serena-bridge undo --format markdown` --phase 03
`gsd-serena-bridge undo --format markdown` --plan 03-02
```
</step>
...
```
Working tree has uncommitted changes. Commit or stash them before running `gsd-serena-bridge undo --format markdown`.
```
...
**Also available:**
- ``gsd-serena-bridge execute-phase --format markdown` ${PHASE}` — re-execute if needed
- ``gsd-serena-bridge undo --format markdown` --last 1` — undo the revert itself if something went wrong
