# Bridge Workflow: health

<!-- generated-by: pnpm gen:bridge-commands -->

## Purpose

This bridge workflow runbook translates the actual GSD-core workflow information and adapts it for Serena bridge operation. Read it when operating `gsd-workflow-health` in a target project.
...
- Contract ID: `gsd-workflow-health`
- Status: `planned`
- Source path: `gsd-core/workflows/health.md`
- Resolved source path: `vendor/reference/gsd-core/gsd-core/workflows/health.md`
- Mirrored runbook path: `.agents/gsd-serena/workflows/health.md`
...
```bash
- Native query translated: `gsd_run query validate.context \` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
...
```bash
- Native query translated: `gsd_run query validate.health $REPAIR_FLAG $BACKFILL_FLAG` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
...
```bash
- Native query translated: `gsd_run query validate.health` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
