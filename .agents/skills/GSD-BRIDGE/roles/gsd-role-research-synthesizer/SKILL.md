---
name: bridge-gsd-role-research-synthesizer
description: "Use when operating the research-synthesizer Serena role workflow in a bridge-installed target project and needing source-informed GSD-safe procedural guidance."
---
...
Use this generated bridge skill as a source-informed runbook for `gsd-role-research-synthesizer`. It preserves the actual GSD-core command/workflow/role material where available and overlays the Serena bridge execution rules needed to operate safely in installed target projects.
...
Recorded path: `agents/gsd-research-synthesizer.md`; resolved path: `vendor/reference/gsd-core/agents/gsd-research-synthesizer.md`.
...
- Native query translated: `# Planning config loaded via gsd-tools query (or gsd-tools.cjs) in commit step` -> use the closest `gsd-serena-bridge <command> --format markdown`, resolver packet, installed contract, or explicit operation-plan step with validation.
```
...
6. **Large-file / truncation fallback.** Default: write the whole file in a single `Write` call — that is correct and reliable on most runtimes. But some runtimes (e.g. OpenCode) cap tool-call output, and a single oversized `Write` is truncated mid-payload ...
...
- Contract ID: `gsd-role-research-synthesizer`
- Family: `role`
- Status: `planned`
...
- `agents/gsd-research-synthesizer.md` (vendor-agent) -> `vendor/reference/gsd-core/agents/gsd-research-synthesizer.md`

### Unsafe Reference Behaviors

- reference tools: Read, Write, Bash
