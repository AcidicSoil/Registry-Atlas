...
- [x] **ITEM-03**: User can view item overview data including title, description, registry namespace, item slug, type, category, taxonomy labels, visual status, and catalog status.
- [x] **ITEM-04**: User can view item technical details including dependencies, dev dependencies, registry dependencies, files, target paths, and escaped raw JSON.
- [x] **ITEM-05**: User can copy shadcn `view` and `add` commands from the item detail surface without the browser executing either command.
